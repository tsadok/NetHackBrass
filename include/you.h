...
struct u_event {
	...
	Bitfield(uplayed_tune,1);	/* opened the drawbridge by passtune */
	Bitfield(invoked,1);		/* invoked Gate to the Sanctum level */
	Bitfield(ufound_amulet,1);	/* found the Amulet of Yendor */
...
#define ROLE_CHAOTIC	AM_CHAOTIC
	...
	xchar attrmax[A_MAX];	/* maximum attributes for this role */
...
struct Race {
	...
	schar attradj[A_MAX];	/* adjustment for maximum allowable attribute */
	struct RoleAdvance hpadv; /* hit point advancement */
	struct RoleAdvance enadv; /* energy advancement */
#if 0	/* DEFERRED */
...
#define LUCKMIN		(-10)
	...
	schar	udaminc;
	schar	uac;
	schar	uspdbon1;	/* speed bonus by light-weight */
	schar	uspdbon2;	/* speed bonus by potion/wand */
#define DAMCAN_RANDOM	0
#define DAMCAN_HEAD	1
#define DAMCAN_FEET	2
#define	DAMCAN_HAND	3
#define	DAMCAN_BODY	4
#define DAMCAN_MAX	9
	uchar	uspellprot;		/* protection by SPE_PROTECTION */
	uchar	usptime;		/* #moves until uspellprot-- */
	uchar	uspmtime;		/* #moves between uspellprot-- */
	schar	uprotection;		/* protection by ring/robe/cloak */
	...
	struct monst *ustuck;
//#ifdef AUTOTHRUST
	struct monst *ulasttgt;	/* last monster you targeted */
//#endif
#ifdef STEED
	struct monst *usteed;
	long ugallop;
	int urideturns;
#endif
	int	ulastuse;		/* role-specific ability */
	int	unextuse;		/* dealy for next use */
	int	uspellit;		/* magic torch */
...
#define Role_special(x) (u.ulastuse && Role_if(x) && !Upolyd)
