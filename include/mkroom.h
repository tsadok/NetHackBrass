...
struct mkroom {
	...
	Bitfield(rndvault,1);	/* the room is random vault */
	Bitfield(needfill,1);	/* need filling (random vault) */
	Bitfield(rsvdbits,6);	/* reserved bits */
	schar reserved;		/* padding */
...
#define D_TEMPLE	2	/* temple-like placement */
	const struct shassortment *assort;
...
#define	EMPTYROOM	14	/* ordinary room but no random objs/mons */
