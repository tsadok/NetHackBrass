...
#define Is_juib_level(x)	(on_level(x, &juiblex_level))
#define Is_ocus_level(x)	(on_level(x, &orcus_level))
...
#define In_Cocyutus(x)		(In_which_hell(x) == INHELL_ASMODEUS)

#define within_bounded_area(X,Y,LX,LY,HX,HY) \
		((X) >= (LX) && (X) <= (HX) && (Y) >= (LY) && (Y) <= (HY))

/* in which part of hell? */
#define INHELL_ASMODEUS		0
#define INHELL_JUIBLEX		1
#define INHELL_BAALZEBUB	2
#define INHELL_ORCUS		3
#define INHELL_HELL		4
#define INHELL_MAZE		5
#define NOT_IN_HELL		-1
