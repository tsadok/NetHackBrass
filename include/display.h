 ...
 * canspotmons(mon)
 *
 * This function is variation of canspotmon() which returns TRUE if
 * you can spot any monsters in the same place as mon's.
 */
#ifndef MONSTEED
#define canspotmons canspotmon
#else
#define canspotmons(mon) \
	(canspotmon(mon) || \
	 (is_mriding(mon) && canspotmon((mon)->mlink)) || \
	 (is_mridden(mon) && canspotmon((mon)->mlink)))
#endif /*MONSTEED*/
 ...
 *
 * object	32(number of materials) for each object.
 *		Lower 5 bits indicate materials.  Count: NUM_OBJECTS << 5
 *
 ...
 *		The beam type is shifted over 4 positions and the direction
 *		is stored in the lower 4 bits.	Count: NUM_ZAP << 4
 *
...
#define GLYPH_RIDING_OFF	(NUMMONS	+ GLYPH_BODY_OFF)
#define GLYPH_RIDDEN_OFF	(NUMMONS	+ GLYPH_RIDING_OFF)
#define GLYPH_STATUE_OFF	(NUMMONS	+ GLYPH_RIDDEN_OFF)
#define GLYPH_OBJ_OFF		((NUMMONS << 5)	+ GLYPH_STATUE_OFF)
#define GLYPH_CMAP_OFF		((NUM_OBJECTS << 5) + GLYPH_OBJ_OFF) /* material patch */
...
#define GLYPH_CLOUD_OFF		(WARNCOUNT	+ GLYPH_WARNING_OFF)
#define MAX_GLYPH		(MAXCCHARS	+ GLYPH_CLOUD_OFF)
...
#define riding_mon_to_glyph(mon) ((int) what_mon(monsndx((mon)->data))+GLYPH_RIDING_OFF)
...
#define obj_to_glyph(obj)						      \
	...
	((otg_temp = random_object()) == CORPSE ?			      \
	 ...
	 (otg_temp == STATUE) ?						      \
	    (random_monster() << 5) + rnd(MAX_MATERIAL) + GLYPH_STATUE_OFF :  \
	    (otg_temp<<5) + rnd(MAX_MATERIAL) + GLYPH_OBJ_OFF)	:	      \
	((obj)->otyp == CORPSE ?					      \
	    (int) (obj)->corpsenm + GLYPH_BODY_OFF :			      \
	 (obj)->otyp == STATUE ?					      \
	    (((int) (obj)->corpsenm) << 5) +				      \
		(obj)->madeof + GLYPH_STATUE_OFF :			      \
	    ((int) (obj)->otyp << 5) +					      \
		((obj)->madeof) + GLYPH_OBJ_OFF))
...
/* Not affected by hallucination.  Gives a generic body for CORPSE (and STATUE) */
#define objnum_to_glyph(onum)	(((int) (onum) << 5) + GLYPH_OBJ_OFF)
...
#define glyph_to_mon(glyph)						\
	...
	glyph_is_riding_monster(glyph) ? ((glyph)-GLYPH_RIDING_OFF) :	\
	glyph_is_ridden_monster(glyph) ? ((glyph)-GLYPH_RIDDEN_OFF) :	\
	glyph_is_statue(glyph) ? (((glyph)-GLYPH_STATUE_OFF)>>5) :	\
	NO_GLYPH)
#define glyph_to_obj(glyph)						\
	(glyph_is_body(glyph) ? CORPSE :				\
	 glyph_is_statue(glyph) ? STATUE :				\
	glyph_is_normal_object(glyph) ? (((glyph)-GLYPH_OBJ_OFF) >> 5) :	\
	NO_GLYPH)
...
#define glyph_is_riding_monster(glyph)					\
    ((glyph) >= GLYPH_RIDING_OFF && (glyph) < (GLYPH_RIDING_OFF+NUMMONS))
...
#define glyph_is_statue(glyph)						\
    ((glyph) >= GLYPH_STATUE_OFF && (glyph) < (GLYPH_STATUE_OFF+(NUMMONS<<5)))
#define glyph_is_normal_object(glyph)					\
    ((glyph) >= GLYPH_OBJ_OFF && (glyph) < (GLYPH_OBJ_OFF+(NUM_OBJECTS<<5)))
#define glyph_is_object(glyph)						\
		(glyph_is_normal_object(glyph)				\
		|| glyph_is_body(glyph) || glyph_is_statue(glyph))
...
#define glyph_is_cloud(glyph)	\
    ((glyph) >= GLYPH_CLOUD_OFF && (glyph) < (GLYPH_CLOUD_OFF + MAXCCHARS))
#endif /* DISPLAY_H */
