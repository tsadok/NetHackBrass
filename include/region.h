...
/*
 * gas cloud/flame/smoke etc.
 */
struct cloud {
	struct cloud *ncloud;
	struct cloud *nchere;	/* next cloud at the same location */
	unsigned c_id;
	xchar cx,cy;
	xchar ctyp;		/* cloud type */

	Bitfield(visible,1);	/* shown on the map */
	Bitfield(doeslit,1);	/* this cloud emits light */
	Bitfield(doesblock,1);	/* cannot see through */
	Bitfield(madebyu,1);	/* hero created the cloud */
	Bitfield(hlev,2);	/* height level */
#define CLOUD_OVERFLOOR	0
#define	CLOUD_OVEROBJ	1
#define	CLOUD_OVERMON	2
	Bitfield(initial,1);	/* used during creation */
	Bitfield(reserved,1);

	int damage;		/* damage rate */
	int density;		/* density of the cloud */
	int glyph;		/* glyph to be displayed */
	int tmp;		/* temporary work */
	long age;		/* creation date */
};

#define	STINKING_CLOUD	1
#define CLOUD_LIGHT	2
