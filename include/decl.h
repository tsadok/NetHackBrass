...
#define mlcgate_level		(dungeon_topology.d_mlcgate_level)
...
E NEARDATA char pl_weapon[PL_PSIZ];
...
#ifdef JP
E char killer_buf_with_separator[BUFSZ];
#endif /*JP*/
...
#define GETOBJ_CHOOSEIT	0x01		/* used callback func from getobj() */
#define GETOBJ_ALLOWALL	0x02
#define GETOBJ_ADDELSE	0x04
#define GETOBJ_ALTSET	0x08
...
E NEARDATA struct monst zeromonst;
...
E struct obj *getobj_override;

/* target of getpos2() */
#define	GETPOS_MON	0x0001
#define GETPOS_OBJ	0x0002
#define	GETPOS_TRAP	0x0004
#define GETPOS_MONTGT	0x0008

/* Bresenham data structure */
struct bresenham {
	int	x, y;		/* current position */
	int	count;		/* */
	int	linetyp;	/* 0:horizontal 1:vertical 2:special */
	int	vx, vy;		/* direction to move */
	int	e;
	int	delta, threshold;
};

#ifdef JP
E NEARDATA struct objdescr jobj_descr[];
E NEARDATA char *jartifact_names[];
E NEARDATA char *jtrnsmon[];
struct getobj_words {
	const char *nani;	/* 0:"âΩ" */
	const char *wo;		/* 0:"Ç" */
	const char *suru;	/* ex. "êHÇ◊ÇÈ" */
	const char *shimasu;	/* ex. "êHÇ◊" */
};
#endif /*JP*/
