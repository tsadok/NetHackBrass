...
#define	ODESC_I_FILE	"objdesc.txt"
#define ODESC_O_FILE	"objdesc"
#define	VAULT_I_FILE	"vault.des"
#define	VAULT_O_FILE	"rndvault.h"
/*JP*/
#define	JOBJ_FILE	"jtrnsobj.dat"
#define	JMON_FILE	"jtrnsmon.dat"
#define JTABLE_FILE	"jtrnstbl.c"
...
static void FDECL(do_objdesc_sub, (long));
static int FDECL(do_objdesc_text, (FILE *, char *));
static void FDECL(chop_newline, (char *));
...
#else /*JP*/
	Sprintf(infile, DATA_IN_TEMPLATE, "j" RUMOR_FILE);
#endif /*JP*/
	Strcat(infile, ".tru");
...
/*    Sprintf(outbuf, "%s %s%s Ver.%s - last build %s.",
	    PORT_ID, DEF_GAME_NAME, subbuf, version_string(versbuf), build_date);*/
    Sprintf(outbuf, "%s %s%s Ver.%s - last build %s.",
	    PORT_ID, DEF_GAME_NAME, subbuf, VARIANTVER, build_date);
    return outbuf;
}
...
do_data()
{
	...
	while (fgets(in_line, sizeof in_line, ifp)) {
	    if (d_filter(in_line)) continue; /* comment */
	    if (((unsigned char)*in_line) > ' ') {	/* got an entry name */
...
#else
	Sprintf(infile, DATA_IN_TEMPLATE, "j" ORACLE_FILE);
#endif /*JP*/
	Strcat(infile, ".txt");
	if (!(ifp = fopen(infile, RDTMODE))) {
		perror(infile);
...
#else
	Sprintf(filename, DATA_IN_TEMPLATE, "j" QTXT_I_FILE);
#endif /*JP*/
	if(!(ifp = fopen(filename, RDTMODE))) {
		perror(filename);
		exit(EXIT_FAILURE);
 ...
 */
int isspace(int);
int split_string(char *line, char *eout, char *jout) {
    char *lastspc = (char *)0;
    char *e, *j;
    char c;
	...
	*e++ = c;
    ...
    do {
	c = *line++;
	if (c == '#' && lastspc) c = 0; /* in-line comment */
	if (isspace(c) || c == ':') {
	    if (!lastspc) lastspc = j;
	} else if (c) {
	    lastspc = (char *)0;
	}
	*j++ = c;
    } while (c);
    if (lastspc) *lastspc = 0;
    return (*eout && *jout);
}
...
do_japanese(void)
{
	int i;
	char *strbuf, *s;
	char enam[BUFSZ], jnam[BUFSZ];
	char infilename[60];
	int nobjs, nmons, narts;
	char class = '\0';

	/* Japanese version of obj_descr[] */
	Sprintf(infilename, DATA_IN_TEMPLATE, JOBJ_FILE);
	if (!(ifp = fopen(infilename, RDTMODE))) {
		perror(infilename);
		exit(EXIT_FAILURE);
	}
	...
	Strcat(filename, file_prefix);
#endif
	Sprintf(eos(filename), SOURCE_TEMPLATE, JTABLE_FILE);
	if (!(ofp = fopen(filename, WRTMODE))) {
		perror(filename);
		Fclose(ifp);
		exit(EXIT_FAILURE);
	}
	Fprintf(ofp,"/*\tSCCS Id: @(#)jtrnstbl.c\t3.4\t2002/02/03 */\n\n");
	Fprintf(ofp,Dont_Edit_Code);
	Fprintf(ofp,"\n");
	Fprintf(ofp,"#include \"hack.h\"\n\n");

	for (nobjs = 1; objects[nobjs].oc_class != ILLOBJ_CLASS; nobjs++) continue;
	for (narts = 1; artifact_names[narts]; narts++) continue;
	strbuf = (char *)malloc(128 * (nobjs + narts)); /* 64 for name, 64 for descr */
	if (strbuf == NULL) {
	...
	for(i = 0; i < (nobjs+narts)*128; i++) strbuf[i] = 0;
	while (fgets(in_line, sizeof in_line, ifp)) {
	    SpinCursor(3);
	    if (d_filter(in_line)) continue;
	    if (!split_string(in_line, enam, jnam)) continue;
	    switch (enam[0]) {
		case WEAPON_SYM:  class = WEAPON_CLASS;	 break;
		case ARMOR_SYM:	  class = ARMOR_CLASS;	 break;
		case RING_SYM:	  class = RING_CLASS;	 break;
		case AMULET_SYM:  class = AMULET_CLASS;	 break;
		case TOOL_SYM:	  class = TOOL_CLASS;	 break;
		case FOOD_SYM:	  class = FOOD_CLASS;	 break;
		case POTION_SYM:  class = POTION_CLASS;	 break;
		case SCROLL_SYM:  class = SCROLL_CLASS;	 break;
		case SPBOOK_SYM:  class = SPBOOK_CLASS;	 break;
		case WAND_SYM:	  class = WAND_CLASS;	 break;
		case GOLD_SYM:	  class = COIN_CLASS;	 break;
		case GEM_SYM:	  class = GEM_CLASS;	 break;
		case ROCK_SYM:	  class = ROCK_CLASS;	 break;
		case BALL_SYM:	  class = BALL_CLASS;	 break;
		case CHAIN_SYM:	  class = CHAIN_CLASS;	 break;
		case VENOM_SYM:	  class = VENOM_CLASS;	 break;
		case 'A':
		    for (i = 1; i< narts; i++) {
			if (!strcmp(&enam[1], artifact_names[i])) {
			    Strcpy(&strbuf[128*(nobjs+i)], jnam);
			}
		    }
		    continue;
		default:
	    ...
	    for (i = 1; i < nobjs; i++) {
		if (objects[i].oc_class != class) continue;
		if (obj_descr[i].oc_name &&
		    !strcmp(&enam[1], obj_descr[i].oc_name)) {
		    Strcpy(&strbuf[128*i], jnam);
		}
		if (obj_descr[i].oc_descr &&
		    !strcmp(&enam[1], obj_descr[i].oc_descr)) {
		    Strcpy(&strbuf[128*i+64], jnam);
		}
	    }
	}
	Fprintf(ofp,"NEARDATA struct objdescr jobj_descr[] = {\n");
	for (i = 0, s = &strbuf[128*i]; i < nobjs; i++, s += 128) {
	    Fprintf(ofp,"    { ");
	    if (*s)
		Fprintf(ofp,"\"%s\", ", s);
	    else {
		Fprintf(ofp,"(char *)0, ");
		if (i && obj_descr[i].oc_name)
		    Fprintf(stdout, "Warning: No Japanese object name for '%s'\n", obj_descr[i].oc_name);
	    }
	    if (s[64])
		Fprintf(ofp,"\"%s\"", &s[64]);
	    else {
		Fprintf(ofp,"(char *)0");
		if (i && obj_descr[i].oc_descr && objects[i].oc_class != SCROLL_CLASS)
		    Fprintf(stdout, "Warning: No Japanese object description for '%s'\n", obj_descr[i].oc_descr);
	    }
	    Fprintf(ofp," },\n");
	}
	Fprintf(ofp,"};\n\n");
	Fprintf(ofp,"NEARDATA char *jartifact_names[] = {\n");
	for (i = 0, s = &strbuf[128*nobjs]; i < narts; i++, s += 128) {
	    if (*s)
		Fprintf(ofp,"    \"%s\",\n", s);
	    else {
		Fprintf(ofp,"    (char *)0,\n");
		if (i)
		    Fprintf(stdout, "Warning: No Japanese artifact name for '%s'\n", artifact_names[i]);
	    }
	}
	Fprintf(ofp,"};\n\n");
	free(strbuf);

	/* Japanese version of monster names */
	Sprintf(infilename, DATA_IN_TEMPLATE, JMON_FILE);
	if (!(ifp = fopen(infilename, RDTMODE))) {
		perror(infilename);
		Fclose(ofp);
		Unlink(filename);	/* kill empty output file */
	...
	for (nmons = 0; mons[nmons].mlet; nmons++) continue;
	strbuf = (char *)malloc(64 * nmons);
	if (strbuf == NULL) {
		perror("Out of memory");
		Fclose(ifp);
		Fclose(ofp);
	...
	for(i = 0; i < nmons*64; i++) strbuf[i] = 0;
	while (fgets(in_line, sizeof in_line, ifp)) {
	    SpinCursor(3);
	    if (d_filter(in_line)) continue;
	    if (!split_string(in_line, enam, jnam)) continue;
	    for (i = 0, s = strbuf; i < nmons; i++, s += 64) {
		if (!strcmp(enam, mons[i].mname)) {
		    Strcpy(s, jnam);
		}
	    }
	}
	Fprintf(ofp,"NEARDATA char *jtrnsmon[] = {\n");
	for (i = 0, s = strbuf; i < nmons; i++, s += 64) {
	    if (*s)
		Fprintf(ofp,"    \"%s\",\n", s);
	    else {
		Fprintf(ofp,"    (char *)0,\n");
		Fprintf(stdout, "Warning: No Japanese monster name for '%s'\n", mons[i].mname);
	    }
	}
	Fprintf(ofp,"};\n\n");
	free(strbuf);
...
/*
	Format of '(j)objdesc.txt' file
<objectA symbol><objectA name>
<objectB symbol><objectB name>
<TAB>Description text for object A and B
<TAB>Description text for object A and B
...

	Format of 'objdesc.dat' file
unsigned long [NUM_OBJECTS*2];		offset to descriptive text
unsigned long [NROFARTIFACTS];
LASCII	text-a				1st line of desc. txt for an object
LASCII	text-a				2nd line
0x00					3rd line (blank line)
LASCII	text-a				4th line
0xFF			end mark
LASCII	text-b				1st line of desc. txt for an object
LASCII	text-b				2nd line
0xFF			end mark
...
 */
static long *obuf;
...
void
do_objdesc_sub(newindex)
long newindex;
{
	if (!odidx) {
	    odidx = newindex;
	} else if (odidx < newindex) {
	    obuf[newindex] = -odidx;
	} else {
	    obuf[odidx] = -newindex;
	    odidx = newindex;
...
char *s;
{
	...
	chop_newline(s);
	c = (unsigned char)*s;
	if (!c) return 0;		/* blank line is assumed as comment */
	if (c <= ' ') {			/* description text */
		len = strlen(s+1);
		fputc(len, fp);
		if (len) fwrite(s+1, sizeof(char), len, fp);
		return len+1;
	}
	return -1;
}
...
do_objdesc(void)
{
...
#ifndef JP
	Sprintf(filename, DATA_IN_TEMPLATE, ODESC_I_FILE);
#else
	Sprintf(filename, DATA_IN_TEMPLATE, "j" ODESC_I_FILE);
#endif /*JP*/
	if(!(ifp = fopen(filename, RDTMODE))) {
		perror(filename);
		exit(EXIT_FAILURE);
	...
	obuf = (long *)malloc(sizeof(long) * (nobjs*2 + narts));
	if (obuf == NULL) {
		perror("Out of memory");
		Fclose(ifp);
		exit(EXIT_FAILURE);
	}
	for (i = 0; i < nobjs*2+narts; i++) obuf[i] = 0;

	filename[0]='\0';
#ifdef FILE_PREFIX
	Strcat(filename, file_prefix);
#endif
	Sprintf(eos(filename), DATA_TEMPLATE, ODESC_O_FILE);
	if(!(ofp = fopen(filename, WRBMODE))) {
		perror(filename);
		Fclose(ifp);
		free(obuf);
	...
	/* output the offset table (with dummy values at this point) */
	(void) fwrite((genericptr_t)obuf, sizeof(long), nobjs*2+narts, ofp);
	woffset = ftell(ofp);

	odidx = 0;
	stt = 0; /* 0:Want index 1:Want txt */
	line_cnt = 0;
	while (fgets(in_line, sizeof in_line, ifp)) {
	    unsigned char c;
	    SpinCursor(3);
	    line_cnt++;
	    if (d_filter(in_line)) continue; /* comment */
	    chop_newline(in_line);
	    c = (unsigned char)*in_line;
	    if (c == '\t') {	/* description text */
		if (!stt) {
		    Fprintf(stderr, "Error in line %4d: Description text before index\n", line_cnt);
		    Fclose(ifp);
		    Fclose(ofp);
		    free(obuf);
		    Unlink(filename);
		...
		if (odidx) {
		    /* This is the 1st line. Record the offset */
		    obuf[odidx] = woffset;
		    odidx = 0;
		}
		woffset += do_objdesc_text(ofp, in_line);
		continue;
	    } else if (c <= ' ') {
		continue;
	    }
	    if (stt) {
		fputc(0xFF, ofp);	    /* end of text */
		woffset++;
	    }
	    switch (c) {
		case WEAPON_SYM:  class = WEAPON_CLASS;	 break;
		...
		case 'A':
		    for (i = 1; i< narts; i++) {
			if (!strcmp(&in_line[1], artifact_names[i])) {
			    do_objdesc_sub((long)(nobjs*2 + i));
			    stt = 1;
			    break;
			}
		    }
		...
		if (objects[i].oc_class != class) continue;
		if (obj_descr[i].oc_name &&
		    !strcmp(&in_line[1], obj_descr[i].oc_name)) {
		    do_objdesc_sub((long)(i*2));
		    stt = 1;
		    break;
		}
		if (obj_descr[i].oc_descr &&
		    !strcmp(&in_line[1], obj_descr[i].oc_descr)) {
		    do_objdesc_sub((long)(i*2+1));
		    stt = 1;
		    break;
		}
	    }
	    if (i < nobjs) continue;
	    Fprintf(stderr, "Error in line %4d: Cannot find \'%s\'\n", line_cnt, &in_line[1]);
	    Fclose(ifp);
	    Fclose(ofp);
	    free(obuf);
	    Unlink(filename);
	...
	for (i = 0; i < nobjs*2+narts; i++ ) {
		offset = obuf[i];
		if (offset >= 0) continue;
		/* share the previous text */
		obuf[i] = obuf[-offset];
	}
	/* overwrite the offset table */
	rewind(ofp);
	(void) fwrite((genericptr_t)obuf, sizeof(long), nobjs*2+narts, ofp);
...
{
	int	i, num = 0;
	int	rarity, minlev, maxlev;
	char	infilename[60];

	Sprintf(infilename, DATA_IN_TEMPLATE, VAULT_I_FILE);
	if (!(ifp = fopen(infilename, RDTMODE))) {
		perror(infilename);
		exit(EXIT_FAILURE);
	}
	...
	Strcat(filename, file_prefix);
#endif
	Sprintf(eos(filename), INCLUDE_TEMPLATE, VAULT_O_FILE);
	if (!(ofp = fopen(filename, WRTMODE))) {
		perror(filename);
		exit(EXIT_FAILURE);
	}
	Fprintf(ofp,"/*\tSCCS Id: @(#)rndvault.h\t3.4\t2002/02/03 */\n\n");
	Fprintf(ofp,Dont_Edit_Code);
	Fprintf(ofp,"#ifndef RNDVAULT_H\n#define RNDVAULT_H\n\n");
	Fprintf(ofp,"struct rndvault {\n");
	Fprintf(ofp,"\tschar rarity;\n");
	Fprintf(ofp,"\tschar minlev;\n");
	Fprintf(ofp,"\tschar maxlev;\n");
	Fprintf(ofp,"} rndvaults[] = {\n");

	while (fgets(in_line, sizeof in_line, ifp)) {
		SpinCursor(3);
		if (sscanf(in_line, "#@ %d %d %d",
			   &rarity, &minlev, &maxlev) == 3) {
			Fprintf(ofp,"\t{ %d, %d, %d },\n", rarity, minlev, maxlev);
			num++;
		}
	}

	Fprintf(ofp,"};\n");
	Fprintf(ofp,"#define NUM_VAULTS %d\n\n", num);
	Fprintf(ofp,"#endif /* RNDVAULT_H */\n");
	Fclose(ofp);
	Fclose(ifp);
	return;
}
