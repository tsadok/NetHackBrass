...
char c;		/* class */
{
	int i, class;
	const char *objname;
	char *sep;		/* material patch */
	char buf[BUFSZ];	/* material patch */
...
/* material patch */
const char *matnames[] = {
	"random", "liquid", "wax", "veggy", "flesh", "paper", "cloth", "leather",
	"wood", "bone", "dragonhide", "iron", "metal", "copper", "silver", "gold",
...
char *s;
{
	...
	s = index(s, ':');
	if (s == NULL) return 0;	/* normal */
	s++;
	for (i = 0; i <= MAX_MATERIAL; i++) {
	    if (!strcmp(s, matnames[i]))
		return (i == 0) ? -127 : i;
	}
	return ERR;
}
...
char *s;
{
	...
	p = index(s, '?');
	if (p == NULL) return;
	for (digit = 0; *p == '?'; p++) digit++;
	sprintf(buf, "%08d", serialno++);
	for (q = &buf[8]; digit; digit--) *(--p) = *(--q);
}
