...
slime_dialogue()
{
	...
	if (((Slimed & TIMEOUT) % 2L) && i >= 0L
		...
		if (i == 4L) {	/* "you are turning green" */
		    ...
		    pline(str, an(Hallucination ? rndmonnam() : E_J("green slime","グリーンスライム")));
	    } else
		pline(str);
	}
...
nh_timeout()
{
	...
	/*Inform Berserking Barbarians they are not berserking -SW */
	if(u.ulastuse == 1 && Role_if(PM_BARBARIAN))
	    E_J(You("calm down a bit."), Your("怒りは収まった。"));
...
#else
			    Strcat(u.usick_cause, "に病気をうつされて死んだ");
			    killer_format = NO_KILLER_PREFIX;
#endif /*JP*/
			}
			u.usick_type = 0;
