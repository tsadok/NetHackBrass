...
ballfall()
{
	...
	if (carried(uball)) {
		pline(E_J("Startled, you drop the iron ball.",
			  "‹Á‚¢‚Ä‚ ‚È‚½‚Í“S‹…‚ğ—‚µ‚½D"));
		if (uwep == uball)
	...
	if(gets_hit){
		int dmg = rn1(7,25);
		pline_The(E_J("iron ball falls on your %s.",
			      "“S‹…‚ª‚ ‚È‚½‚Ì%s‚ß‚ª‚¯‚Ä~‚Á‚Ä‚«‚½B"),
			body_part(HEAD));
		if (uarmh) {
		    if(is_metallic(uarmh)) {
			pline(E_J("Fortunately, you are wearing a hard helmet.",
				  "K‚¢A‚ ‚È‚½‚Íd‚¢Š•‚ğ‚©‚Ô‚Á‚Ä‚¢‚½B"));
			dmg = 3;
		    } else if (flags.verbose)
			Your(E_J("%s does not protect you.",
				 "%s‚Å‚Í“S‹…‚ğ–h‚°‚È‚©‚Á‚½B"), xname(uarmh));
		}
		losehp(dmg, E_J("crunched in the head by an iron ball",
				"“S‹…‚É“ª‚ğ’×‚³‚ê‚Ä"),
...
drag:
	...
	if ((is_pool(uchain->ox, uchain->oy) &&
	    ...
	    if (Levitation) {
		You_feel(E_J("a tug from the iron ball.",
			     "“S‹…‚Éˆø‚Á‚Ï‚ç‚ê‚½B"));
		if (t) t->tseen = 1;
	    } else {
		struct monst *victim;

		You(E_J("are jerked back by the iron ball!",
			"“S‹…‚Éˆø‚«–ß‚³‚ê‚½I"));
...
xchar x, y;
{
    ...
    if (x != u.ux || y != u.uy) {
	struct trap *t;
	const char *pullmsg = E_J("The ball pulls you out of the %s!",
				  "“S‹…‚ª‚ ‚È‚½‚ğ%s‚©‚çˆø‚Á‚Ï‚èo‚µ‚½I");
	...
	if (u.utrap && u.utraptype != TT_INFLOOR) {
		...
		pline_The(E_J("web is destroyed!","’wå‚Ì‘ƒ‚Íˆø‚«‚¿‚¬‚ê‚½I"));
		deltrap(t_at(u.ux,u.uy));
		break;
	    case TT_LAVA:
		pline(pullmsg, E_J("lava","—nŠâ"));
		break;
	    case TT_BEARTRAP: {
		register long side = rn2(3) ? LEFT_SIDE : RIGHT_SIDE;
		pline(pullmsg, E_J("bear trap","ƒgƒ‰ƒoƒTƒ~"));
		set_wounded_legs(side, rn1(1000, 500));
#ifdef STEED
		if (!u.usteed)
#endif
		{
		    Your(E_J("%s %s is severely damaged.",
			     "%s%s‚Í‚Ğ‚Ç‚­‚Â‚¢‚½B"),
					(side == LEFT_SIDE) ? E_J("left","¶") : E_J("right","‰E"),
					body_part(LEG));
		    losehp(2, E_J("leg damage from being pulled out of a bear trap",
				  "ƒgƒ‰ƒoƒTƒ~‚©‚ç‹r‚ğ–³—‚Éˆø‚«”‚ª‚µ‚½‚Æ‚«‚Ì‰ö‰ä‚Å"),
					KILLED_BY);
		}
		break;
	      }
...
drag_down()
{
	...
	if (carried(uball))
		You(E_J("lose your grip on the iron ball.",
			"“S‹…‚ğˆ¬‚Á‚Ä‚¢‚ç‚ê‚¸A—‚Æ‚µ‚Ä‚µ‚Ü‚Á‚½B"));

	if (forward) {
		if(rn2(6)) {
			pline_The(E_J("iron ball drags you downstairs!",
				      "“S‹…‚ª‚ ‚È‚½‚ğˆø‚«‚¸‚Á‚ÄŠK’i‚ğ—‚¿‚Ä‚ä‚­I"));
			losehp(rnd(6), E_J("dragged downstairs by an iron ball",
					   "“S‹…‚ÉŠK’i‚ğˆø‚«‚¸‚è—‚Æ‚³‚ê‚Ä"),
				E_J(NO_KILLER_PREFIX,KILLED_BY));
			litter();
		}
		...
		if(rn2(2)) {
			pline_The(E_J("iron ball smacks into you!",
				      "“S‹…‚ª‚ ‚È‚½‚ÉŒƒ“Ë‚µ‚½I"));
			losehp(rnd(20), E_J("iron ball collision","“S‹…‚É‚Ô‚Â‚©‚Á‚Ä"), KILLED_BY_AN);
		...
		if( (int) dragchance >= rnd(6)) {
			pline_The(E_J("iron ball drags you downstairs!",
				      "“S‹…‚ª‚ ‚È‚½‚ğˆø‚«‚¸‚Á‚ÄŠK’i‚ğ—‚¿‚Ä‚ä‚­I"));
			losehp(rnd(3), E_J("dragged downstairs by an iron ball",
					   "“S‹…‚ÉŠK’i‚ğˆø‚«‚¸‚è—‚Æ‚³‚ê‚Ä"),
				E_J(NO_KILLER_PREFIX,KILLED_BY));
			exercise(A_STR, FALSE);
			litter();
