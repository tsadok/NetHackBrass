...
/*  shop name				shop type    prob         assortment		   shk name */
  {E_J("general store","éGâ›ìX"),	RANDOM_CLASS, 42, D_SHOP, generalstore_assortment, shkgeneral},
  {E_J("used armor dealership","íÜå√äZîÃîÑìX"),
					ARMOR_CLASS,  14, D_SHOP, armorshop_assortment,    shkarmors},
  {E_J("second-hand bookstore","å√èëìX"),
					SCROLL_CLASS, 10, D_SHOP, scrollshop_assortment,   shkbooks},
  {E_J("liquor emporium","Ç®éÇÃïSâ›ìX"),
					POTION_CLASS, 10, D_SHOP, liquorshop_assortment,   shkliquors},
  {E_J("antique weapons outlet","çúìüïêäÌè¨îÑìX"),
					WEAPON_CLASS,  5, D_SHOP, weaponshop_assortment,   shkweapons},
  {E_J("delicatessen","êHïiäŸ"),	FOOD_CLASS,    5, D_SHOP, foodshop_assortment,     shkfoods},
  {E_J("jewelers","ïÛè¸ìX"),		RING_CLASS,    3, D_SHOP, jewelers_assortment,     shkrings},
  {E_J("quality apparel and accessories","çÇãâómïiìX"),
					WAND_CLASS,    3, D_SHOP, wandshop_assortment,     shkwands},
  {E_J("hardware store","ìπãÔìX"),	TOOL_CLASS,    3, D_SHOP, toolshop_assortment,     shktools},
	/* Actually shktools is ignored; the code specifically chooses a
	 * random implementor name (along with candle shops having
	 * random shopkeepers)
	 */
  {E_J("rare books","ãHÊQñ{êÍñÂìX"),	SPBOOK_CLASS,  3, D_SHOP, bookshop_assortment,     shkbooks},
  {E_J("stylish clothing store","ëfìGÇ»ïûè¸ìX"),
	 ...
	 * loader.
	 */
  {E_J("lighting store","è∆ñæäÌãÔìX"),	TOOL_CLASS,    0, D_SHOP, candleshop_assortment,   shklight},
  {(char *)0,				0,	       0, 0,	  0,			   0}
};
...
# endif
	...
	ESHK(shk)->billct = 0;
	ESHK(shk)->floorgold.x = -1;
	ESHK(shk)->dosweep = FALSE;
#ifndef GOLDOBJ
...
register struct mkroom *sroom;
{
    ...
    if(levl[sx][sy].doormask == D_LOCKED) {
	    ...
	    Sprintf(buf, E_J("Closed for inventory","íIâµÇÃÇΩÇﬂï¬ìXíÜ"));
