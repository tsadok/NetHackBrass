    ...
    struct obj *weap;
{
	...
	if (flags.mon_moving)
	    pline(E_J("Part of the tail of %s is cut off.",
		      "%sの尾の一部が切り落とされた。"), mon_nam(worm));
	else
	    You(E_J("cut part of the tail off of %s.",
		    "%sの尾の一部を切り落とした。"), mon_nam(worm));
    ...
    if (flags.mon_moving)
	pline(E_J("%s is cut in half.","%sは真っ二つにされた。"), Monnam(worm));
    else
	You(E_J("cut %s in half.","%sを真っ二つにした。"), mon_nam(worm));
}
