...
dosave()
{
	...
	if(yn(E_J("Really save?","本当にセーブしますか？")) == 'n') {
		clear_nhwindow(WIN_MESSAGE);
		if(multi > 0) nomul(0);
	} else {
		clear_nhwindow(WIN_MESSAGE);
		pline(E_J("Saving...","セーブ中…。"));
		...
		if(dosave0()) {
			...
			exit_nhwindows(E_J("Be seeing you...","またお会いしましょう…。"));
...
dosave0()
{
...
#else
		pline("古いセーブファイルがあるようだ。");
#endif /*JP*/
		if (yn(E_J("Overwrite the old file?","上書きしますか？")) == 'n') {
		    compress(fq_save);
		    return 0;
		}
	    }
	...
	fd = create_savefile();
	if(fd < 0) {
		HUP pline(E_J("Cannot open save file.","セーブファイルを開けません。"));
...
savestateinlock()
{
	...
	if (flags.ins_chkpt || havestate) {
		...
		if (hackpid != hpid) {
		    Sprintf(whynot,
			    E_J("Level #0 pid (%d) doesn't match ours (%d)!",
...
int mode;
{
 ...
 skip_lots:
	...
	save_cloud(fd, level.cloudlist, mode);
	if (release_data(mode))
	    level.cloudlist = 0;
...
register struct monst *mtmp;
{
	...
	while (mtmp) {
	    mtmp2 = mtmp->nmon;
	    if (perform_bwrite(mode)) {
		/* replace monst pointers to id */
		if ((msvp = mtmp->mlink) != 0)
		    mtmp->mlink = (genericptr_t) (mtmp->mlink)->m_id;

		xl = mtmp->mxlth + mtmp->mnamelth;
		bwrite(fd, (genericptr_t) &xl, sizeof(int));
		bwrite(fd, (genericptr_t) mtmp, xl + sizeof(struct monst));

		/* restore monst pointers */
		mtmp->mlink = msvp;
	    }
	    if (mtmp->minvent)
...
# define freeclouds(X)	(save_cloud(0, X, FREE_SAVE),  X = 0)
