...
guardname()	/* return your role leader's guard monster name */
{
	...
	return(E_J(mons[i].mname, JMONNAM(i)));
}
...
char c;
{
	...
	switch (c) {
	    ...
	    case 'R':	str = rank_of(MIN_QUEST_LEVEL, Role_switch,
	    			flags.female);
			break;
	    case 's':	str = (flags.female) ? E_J("sister","妹") : E_J("brother","弟");
			break;
	    case 'S':	str = (flags.female) ? E_J("daughter","娘") : E_J("son","息子");
			break;
	    case 'l':	str = ldrname();
			break;
	    case 'i':	str = intermed();
			break;
	    case 'o':	str = E_J(the(artiname(urole.questarti)), artiname(urole.questarti));
			break;
	    case 'n':	str = neminame();
			break;
	    ...
	    case 'D':	str = align_gname(A_LAWFUL);
			break;
	    case 'C':	str = E_J("chaotic", "混沌");
			break;
	    case 'N':	str = E_J("neutral", "中立");
			break;
	    case 'L':	str = E_J("lawful", "秩序");
			break;
	    case 'x':	str = Blind ? E_J("sense","感じられた") : E_J("see","見えた");
			break;
	    case 'Z':	str = E_J(dungeons[0].dname, jdgnnam(0));
			break;
	    case '%':	str = "%";
			break;
	     default:	str = "";
