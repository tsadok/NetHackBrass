...
#endif
#if defined(mac) || defined(__BEOS__) || defined(WIN32_GRAPHICS) || defined(WIN32_GRAPHICS2)
...
"                  |       1001       |", /* Real year of death */
"                 *|     *  *  *      | *",
"        _________)/Б╔_//(V/(/V)/V//V/|_)_______",
#endif /*JP*/
0
};
...
#else
	jprepare_epitaph(buf, 16);
	if (0 == *(jget_epitaph(2))) {
	    int i;
	    for (i=0; i<3; i++)
		center(DEATH_LINE+1+i, jget_epitaph(i));
	} else
	    for (line=DEATH_LINE; line<YEAR_LINE; line++)
		center(line, jget_epitaph(line - DEATH_LINE));
#endif /*JP*/
