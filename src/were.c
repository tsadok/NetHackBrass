...
register struct monst *mon;
{
	...
	if (is_human(mon->data)) {
		...
		if (flags.soundok && !canseemon(mon)) {
		    ...
		    case PM_WEREWOLF:	howler = E_J("wolf",  "狼");	      break;
		    case PM_WEREJACKAL: howler = E_J("jackal","ジャッカル");  break;
		    default:		howler = (char *)0; break;
		    }
		    if (howler)
			You_hear(E_J("a %s howling at the moon.",
				     "%sが月に向かって吠えるのを"), howler);
		}
	    }
...
#ifdef JP
STATIC_DCL const char *FDECL(jweretypnam, (int));
#endif /*JP*/
...
int pm;
{
	switch(pm) {
	    case PM_WEREWOLF:
	    case PM_HUMAN_WEREWOLF:	return "狼";
	    case PM_WEREJACKAL:
	    case PM_HUMAN_WEREJACKAL:	return "ジャッカル";
	    case PM_WERERAT:
	    case PM_HUMAN_WERERAT:	return "ネズミ";
	    default:
		impossible("jwerenam(): おかしなWere怪物");
		return 0;
	}
}
#endif /*JP*/
...
register struct monst *mon;
{
	...
	if(canseemon(mon) && !Hallucination)
	    ...
	    pline("%s changes into a %s.", Monnam(mon),
			...
			mons[pm].mname+4);
#else
	    pline("%sは%sに変身した。", Monnam(mon),
		  is_human(&mons[pm]) ? "人間" : jweretypnam(pm));
#endif /*JP*/
	...
	for(i = rnd(5); i > 0; i--) {
	   switch(pm) {
		...
		case PM_HUMAN_WERERAT:
			...
			if (genbuf) Strcpy(genbuf, E_J("rat","ネズミ"));
			break;
		case PM_WEREJACKAL:
		case PM_HUMAN_WEREJACKAL:
			typ = PM_JACKAL;
			if (genbuf) Strcpy(genbuf, E_J("jackal","ジャッカル"));
			break;
		case PM_WEREWOLF:
		case PM_HUMAN_WEREWOLF:
			typ = rn2(5) ? PM_WOLF : PM_WINTER_WOLF ;
			if (genbuf) Strcpy(genbuf, E_J("wolf","狼"));
			break;
		default:
			continue;
	    }
...
{
	if (purify) {
	    You_feel(E_J("purified.","清められた気がした。"));
	    u.ulycn = NON_PM;	/* cure lycanthropy */
	}
	if (!Unchanging && is_were(youmonst.data) &&
		(!Polymorph_control ||
		 yn(("Remain in beast form?","獣の姿にとどまりますか？")) == 'n'))
	    rehumanize();
}
