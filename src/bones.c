...
struct obj *cont;
{
	...
	while ((otmp = invent) != 0) {
		...
		/* make it rusty/corroded/rot */
		otmp->oerodeproof = 0;
		if (is_rustprone(otmp) && is_corrodeable(otmp) &&
		    (otmp->oclass == WEAPON_CLASS ||
		     otmp->oclass == ARMOR_CLASS)) {
		    if (!rn2(2)) otmp->oeroded  = rnd(MAX_ERODE);
		    else	 otmp->oeroded2 = rnd(MAX_ERODE);
...
struct obj *corpse;
{
 ...
 make_bones:
...
#else
		Your("Ž€‘Ì‚Í%s‚Æ‚È‚Á‚Ä“®‚«o‚µ‚½cB",
			JMONNAM(u.ugrave_arise));
#endif /*JP*/
	...
	Strcpy(newbones->when, yyyymmdd(when));
