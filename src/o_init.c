	...
	boolean domaterial;
{
	...
	for (j=o_low; j <= o_high; j++) {
		...
		if (domaterial) {	/* material patch */
			sw = objects[j].oc_other_material;
			objects[j].oc_other_material = objects[i].oc_other_material;
			objects[i].oc_other_material = sw;
		}
...
#ifdef MAGIC_GLASSES
	/* shuffle the glasses */
	shuffle(GLASSES_OF_MAGIC_READING, GLASSES_OF_PHANTASMAGORIA, FALSE);
#endif
}
...
dodiscovered()				/* free after Robert Viduya */
{
    ...
    putstr(tmpwin, 0, E_J("Discoveries","”­Œ©•¨ˆê——"));
    putstr(tmpwin, 0, "");
	...
	if (objects[uniq_objs[i]].oc_name_known) {
	    if (!dis++)
		putstr(tmpwin, iflags.menu_headings, E_J("Unique Items","“Á•Ê‚È•i•¨"));
	    Sprintf(buf, "  %s", E_J(OBJ_NAME(objects[uniq_objs[i]]),
				    JOBJ_NAME(objects[uniq_objs[i]])));
#ifdef JP
	    if (i == 0/*AoY*/) Strcpy(eos(buf), "–‚œ‚¯");
#endif /*JP*/
	    putstr(tmpwin, 0, buf);
	    ++ct;
	}
