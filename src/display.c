    ...
    int  show;
{
	...
	if (HAS_LITGLYPH(lev->typ) && is_litcmap(glyph_to_cmap(glyph)))
	    glyph--;
    ...
    register int  show;
{
    ...
    if (show && show_cloud(x,y,CLOUD_OVERFLOOR)) return;
    if (show) show_glyph(x,y, glyph);
}
...
{									\
    ...
    register struct trap  *trap;					\
									\
    if (show && show_cloud(x,y,CLOUD_OVEROBJ));				\
    ...
    xchar x, y;
{
	...
	if (IS_ROCK(lev->typ) || (IS_DOOR(lev->typ) &&
		...
		if (!HAS_LITGLYPH(lev->typ) && lev->seenv) {
		    map_background(x, y, 1);
		} else {
		    int bak_litcorr = flags.lit_corridor;
		    flags.lit_corridor = 0;
		    lev->glyph = back_to_glyph(x, y);
		    flags.lit_corridor = bak_litcorr;
...
//					       cmap_to_glyph(S_stone);
	...
	/* Floor spaces are dark if unlit.  Corridors are dark if unlit. */
	if (HAS_LITGLYPH(lev->typ) && lev->glyph == back_to_glyph(x,y) && !lev->waslit)
	    show_glyph(x,y, --(lev->glyph));
//	if (lev->typ == ROOM &&
    ...
    register int x,y;
{
	...
	/* show cloud over monster */
	if (show_cloud(x, y, CLOUD_OVERMON)) {
	    if (x == u.ux && y == u.uy && senseself())
		display_self();
	    return;
	}
	if (x == u.ux && y == u.uy) {
...
#ifdef MONSTEED
	    else if (mon && is_mriding(mon) && (mon2 = mon->mlink) &&
		     ((see_it = (mon_visible(mon2) || tp_sensemon(mon2) ||
			 MATCH_WARN_OF_MON(mon2))) || Detect_monsters)) {
		if (mon->mtrapped) {
		    struct trap *trap = t_at(x, y);
		    int tt = trap ? trap->ttyp : NO_TRAP;
		...
		_map_location(x,y,0);	/* map under the monster */
		/* also gets rid of any invisibility glyph */
		display_monster(x, y, mon2, see_it ? PHYSICALLY_SEEN : DETECTED, FALSE);
	    }
#endif /*MONSTEED*/
	...
	if (x == u.ux && y == u.uy) {
	...
	else if (mon && ((see_it = (tp_sensemon(mon) || MATCH_WARN_OF_MON(mon)
	    ...
	    display_monster(x, y, mon, see_it ? 0 : DETECTED, 0);
		 ...
		 ((see_it = (tp_sensemon(mon2) || MATCH_WARN_OF_MON(mon2)
		    		|| (see_with_infrared(mon2) && mon_visible(mon2))))
		    || Detect_monsters)) {
	    display_monster(x, y, mon2, see_it ? 0 : DETECTED, 0);
	}
#endif /*MONSTEED*/
	else if (mon && mon_warning(mon) && !is_worm_tail(mon)) {
	        display_warning(mon);
	 ...
	 */
	    ...
	    if (is_litcmap(glyph_to_cmap(lev->glyph)) && HAS_LITGLYPH(lev->typ))
		show_glyph(x, y, --(lev->glyph));
    ...
    int x, y, glyph;
{
	...
	if (glyph >= GLYPH_CLOUD_OFF) {			/* cloud */
	...
	} else if (glyph >= GLYPH_STATUE_OFF) {		/* statue */
	    text = "statue";		offset = glyph - GLYPH_OBJ_OFF;
	} else if (glyph >= GLYPH_RIDDEN_OFF) {		/* ridden mon */
	    text = "ridden mon";	offset = glyph - GLYPH_RIDDEN_OFF;
	} else if (glyph >= GLYPH_RIDING_OFF) {		/* riding mon */
	    text = "riding mon";	offset = glyph - GLYPH_RIDING_OFF;
    ...
    xchar x,y;
{
	...
	case POOL:
	case MOAT:
	    idx = (ptr->waslit || flags.lit_corridor) ? S_litpool : S_pool;
	    break;
	case STAIRS:
	...
	case ICE:
	    idx = (ptr->waslit || flags.lit_corridor) ? S_litice : S_ice;
	    break;
	case AIR:		idx = S_air;	  break;
	case CLOUD:		idx = S_cloud;	  break;
	case WATER:		idx = S_water;	  break;
	case BOG:
	    idx = (ptr->waslit || flags.lit_corridor) ? S_litbog : S_bog;
	    break;
	case GROUND:
	    idx = (ptr->waslit || flags.lit_corridor) ? S_litground : S_ground;
	    break;
	case GRASS:
	    idx = (ptr->waslit || flags.lit_corridor) ? S_litgrass : S_grass;
	    break;
	case CARPET:
	    idx = (ptr->waslit || flags.lit_corridor) ? S_litcarpet : S_carpet;
	    break;
	case DBWALL:
	...
	case DRAWBRIDGE_UP:
	    ...
	    case DB_MOAT:  idx = (ptr->waslit || flags.lit_corridor) ? S_litpool : S_pool; break;
	    case DB_LAVA:  idx = S_lava; break;
	    case DB_ICE:   idx = (ptr->waslit || flags.lit_corridor) ? S_litice : S_ice; break;
	    case DB_FLOOR: idx = S_room; break;
	    case DB_BOG:   idx = (ptr->waslit || flags.lit_corridor) ? S_litbog : S_bog; break;
	    case DB_GROUND:idx = (ptr->waslit || flags.lit_corridor) ? S_litground : S_ground; break;
	    default:
...
//    dx = (dx == dy) ? 2 : (dx && dy) ? 3 : dx ? 1 : 0;
    ...
    return ((int) ((beam_type << 4) | zsym[dxdy2dir(dx, dy) & 3])) + GLYPH_ZAP_OFF;
}
