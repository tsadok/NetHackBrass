...
static const struct { const char *txt; int nut; } tintxts[] = {
	{E_J("deep fried",	"%sの唐揚げ"),		 60},
	{E_J("pickled",		"%sの酢漬け"),		 40},
	{E_J("soup made from",	"%sのスープ"),		 20},
	{E_J("pureed",		"%sのピューレ"),	500},
#define ROTTEN_TIN 4
	{E_J("rotten",		"腐った%sの缶詰"),	-50},
#define HOMEMADE_TIN 5
	{E_J("homemade",	"自家製の%sの缶詰"),	 50},
	{E_J("stir fried",	"%sの炒め物"),		 80},
	{E_J("candied",		"%sの甘露煮"),		100},
	{E_J("boiled",		"%sの煮物"),		 50},
	{E_J("dried",		"%sの干物"),		 55},
	{E_J("szechuan",	"四川風%s"),		 70},
#define FRENCH_FRIED_TIN 11
	{E_J("french fried",	"%sのフライ"),		 40},
	{E_J("sauteed",		"%sのソテー"),		 95},
	{E_J("broiled",		"%sの炭火焼き"),	 80},
	{E_J("smoked",		"%sの燻製"),		 50},
	{"", 0}
};
	...
	register struct obj *food;
{
	...
	if (u.uhs != SATIATED) {
		if (!food || food->otyp != AMULET_OF_STRANGULATION)
			...
			You_feel(E_J("like a glutton!","暴食の愚を犯した気がした！"));
	}
	...
	if (Breathless || (!Strangled && !rn2(20))) {
		 ...
		 */
		if(food) {
			You(E_J("choke over your %s.", "%sを喉に詰まらせた。"), foodword(food));
			if (food->oclass == COIN_CLASS) {
				killer = E_J("a very rich meal","とても高価な食事で");
			} else {
			...
			You(E_J("choke over it.","喉を詰まらせた。"));
			killer = E_J("quick snack","つまみ食いをして");
		}
		You(E_J("die...","死にました…。"));
		done(CHOKING);
	}
...
boolean message;
{
	...
	if (nomovemsg) {
		...
		You(E_J("finish eating %s.","%sを食べ終えた。"), food_xname(victual.piece, TRUE));
...
#else
		Sprintf(killer_buf, "%sの肉を味見して", JMONNAM(pm));
#endif /*JP*/
		killer_format = KILLED_BY;
		killer = killer_buf;
	...
	switch(pm) {
...
#else
		    pline("%sを食べるのは良くないことだと思われた。", JMONNAM(pm));
#endif
		    HAggravate_monster |= FROMOUTSIDE;
		}
		break;
		...
		{ char buf[BUFSZ];
		    ...
		    Sprintf(buf, "unwisely ate the body of %s",
			    mons[pm].mname);
#else
		    Sprintf(buf, "愚かにも%sの死体を食べて", JMONNAM(pm));
#endif /*JP*/
		    killer = buf;
		...
		if (!Slimed && !Unchanging && !flaming(youmonst.data) &&
		    ...
		    You(E_J("don't feel very well.","とても気分が悪い。"));
		    Slimed = 10L;
		    flags.botl = 1;
		}
		/* Fall through */
...
fix_petrification()
{
	...
	if (Hallucination)
	    pline(E_J("What a pity - you just ruined a future piece of %sart!",
		      "なんてこった ─ 後世に残る%s芸術作品になれたのに！"),
		  ACURR(A_CHA) > 15 ? E_J("fine ","素晴らしい") : "");
	else
	    You_feel(E_J("limber!","より柔らかくなったようだ！"));
}
...
register struct permonst *ptr;
{
	...
	switch (type) {
...
#else /*JP*/
			if (Hallucination) pline("あんた、クールだぜ。");
			else You("一瞬冷たさを感じた。");
#endif /*JP*/
		...
		if(!(HSleep_resistance & FROMOUTSIDE)) {
			You_feel(E_J("wide awake.","すっかり目が覚めた。"));
		...
		if(!(HCold_resistance & FROMOUTSIDE)) {
			You_feel(E_J("full of hot air.","全身に熱気を感じた。"));
		...
		if(!(HDisint_resistance & FROMOUTSIDE)) {
			You_feel(Hallucination ?
			    E_J("totally together, man.","ホントに一つになったみたいだ。") :
			    E_J("very firm.","とても頑丈になったようだ。"));
		...
		if(!(HShock_resistance & FROMOUTSIDE)) {
			if (Hallucination)
				You_feel(E_J("grounded in reality.","実はアースされている気がした。"));
			else
		...
		if(!(HPoison_resistance & FROMOUTSIDE)) {
			You_feel(Poison_resistance ?
				 E_J("especially healthy.","きわめて健康になった気がした。") :
				 E_J("healthy.","健康になった気がした。"));
		...
		if(!(HTeleportation & FROMOUTSIDE)) {
			You_feel(Hallucination ? E_J("diffuse.","周りに拡散していく気がした。") :
			    E_J("very jumpy.","とても落ち着かなくなった。"));
		...
		if(!(HTeleport_control & FROMOUTSIDE)) {
			You_feel(Hallucination ?
			    E_J("centered in your personal space.","自分のパーソナル・スペースの中心にいる気がした。") :
			    E_J("in control of yourself.","自分自身を制御できるようになった気がした。"));
		...
		if(!(HTelepat & FROMOUTSIDE)) {
			You_feel(Hallucination ?
			    E_J("in touch with the cosmos.","宇宙の神秘に触れたような気がした。") :
			    E_J("a strange mental acuity.","精神が不思議と鋭敏になるのを感じた。"));
...
register int pm;
{
	...
	switch(pm) {
		...
		if (rn2(3) || 3 * u.uen <= 2 * u.uenmax) {
		    ...
		    if (old_uen != u.uen) {
			    You_feel(E_J("a mild buzz.","静かな高まりを感じた。"));
			    flags.botl = 1;
		    }
		}
		break;
/* STEHPEN WHITE'S NEW CODE */
		...
		if(!Invis) {
			...
			if (!(HInvis & INTRINSIC)) You_feel(E_J("hidden!","身を隠した気分になった！"));
			HInvis |= FROMOUTSIDE;
...
#else
		    You("%sに化けたいという欲求を抑えきれなくなった！",
			Hallucination ? "みかん" : "金塊の山");
#endif /*JP*/
#ifdef STEED
...
#else
		    Sprintf(buf, Hallucination ?
			"突然、あなたは皮をむかれるのが怖くなり、再び%sのふりをした！" :
			"あなたはやはり%sのふりをすることにした。",
		...
		if (HFast & INTRINSIC) {
			HFast &= ~INTRINSIC;
			You(E_J("seem slower.","遅くなったようだ。"));
		} else {
			HFast |= FROMOUTSIDE;
			You(E_J("seem faster.","速くなったようだ。"));
		}
		break;
	    case PM_LIZARD:
	 ...
	 /* case PM_SANDESTIN: */
	    ...
	    case PM_DISENCHANTER:
		{
			...
			if ((otmp->spe <
				...
				pline_The(E_J("blue glow converges on your %s, and disappears.",
					      "青い光はあなたの%sに収束し、そして消えた。"), xname(otmp));
			    else
#ifndef JP
				Your("%s %s warm for a moment.",
					xname(otmp), otense(otmp, "feel"));
#else
				Your("%sが一瞬暖かくなった。", xname(otmp));
#endif /*JP*/
			    otmp->spe++;
			    adj_abon(otmp, 1);
			    update_inventory();
			    ...
			    if (!Blind)
				pline_The(E_J("blue glow disappears.","青い光は消えた。"));
			}
		    }
		}
		break;
		...
		if (dmgtype(ptr, AD_STUN) || dmgtype(ptr, AD_HALU) ||
		    pm == PM_VIOLET_FUNGUS) {
			pline (E_J("Oh wow!  Great stuff!","うわあ！ こりゃすごいや！"));
	...
	const char* verb;		/* if 0, the verb is "open" */
{
	if(((!carried(tin.tin) &&
	    ...
	    verbalize(E_J("You %s it, you bought it!",
			  "%s買ってもらうよ！"), verb ? verb : E_J("open","開けたのなら"));
...
opentin()		/* called during each move whilst opening a tin */
{
	...
	if(tin.tin->otrapped ||
	   (tin.tin->cursed && tin.tin->spe != -1 && !rn2(8))) {
		b_trapped(E_J("tin","缶詰"), 0);
		costly_tin(E_J("destroyed","台無しにしたんだから、"));
		goto use_me;
	}
	You(E_J("succeed in opening the tin.","缶を開けることに成功した。"));
	if(tin.tin->spe != 1) {
	    if (tin.tin->corpsenm == NON_PM) {
		pline(E_J("It turns out to be empty.","中には何も入っていなかった。"));
		...
		if (isstoneorslime(tin.tin->corpsenm)) {
			...
			pline(E_J("It could be foul and unfamiliar to you.",
				  "これは全く馴染みのない、不快なもののようだ。"));
			dangerfood = TRUE;
		    }
		}
	    }
	    Sprintf(buf, E_J("Eat it%s?","%s食べますか？"), dangerfood ? E_J(" anyway","それでも") : "");
	    if (yn(buf) == 'n') {
		if (!Hallucination) tin.tin->dknown = tin.tin->known = TRUE;
		if (flags.verbose) You(E_J("discard the open tin.","開けた缶詰を捨てた。"));
		costly_tin((const char*)0);
		goto use_me;
	    }
...
#else
		pline("揚げ物を食べたせいで、あなたの%sは滑りやすくなった。",
			body_part(FINGER));
#endif /*JP*/
	    }
	} else {
...
#else
		pline("中には何か腐った%s物体が入っている。", Blind ? "" : hcolor(NH_GREEN));
#endif /*JP*/
	    else
		pline(E_J("It contains spinach.","ほうれん草が入っている。"));

	    if (yn(E_J("Eat it?","食べますか？")) == 'n') {
		if (!Hallucination && !tin.tin->cursed)
		    tin.tin->dknown = tin.tin->known = TRUE;
		if (flags.verbose)
		    You(E_J("discard the open tin.","開けた缶詰を捨てた。"));
		costly_tin((const char*)0);
		goto use_me;
	    }
	    ...
	    if (!tin.tin->cursed)
		pline(E_J("This makes you feel like %s!","%sのような気分だ！"),
		      Hallucination ? E_J("Swee'pea","スウィーピー") : E_J("Popeye","ポパイ"));
	    else pline(E_J("Yuck. You feel sick.","オエッ。あなたは気持ち悪くなった。"));
	    lesshungry(600);
	    gainstr(tin.tin, 0);
	    u.uconduct.food++;
	}
	...
	register struct obj *otmp;
{
	...
	if (metallivorous(youmonst.data)) {
		...
		You(E_J("cannot handle the tin properly to open it.",
			"缶を開ける作業を正しくこなすことができない。"));
		return;
	} else if (otmp->blessed) {
		pline_The(E_J("tin opens like magic!","缶はまるで魔法のように開いた！"));
		tmp = 1;
	} else if(uwep) {
		switch(uwep->otyp) {
		case TIN_OPENER:
...
no_opener:
		pline(E_J("It is not so easy to open this tin.",
			  "この缶詰を開けるのは容易なことではない。"));
		if(Glib) {
			pline_The(E_J("tin slips from your %s.","缶詰はあなたの%sからすべり落ちた。"),
	...
	tin.usedtime = 0;
	tin.tin = otmp;
	set_occupation(opentin, E_J("opening the tin","缶詰を開けるの"), 0);
	return;
}
...
struct obj *obj;
{
	pline(E_J("Blecch!  Rotten %s!","おえぇ！ この%sは腐っている！"), foodword(obj));
	if(!rn2(4)) {
		if (Hallucination) You_feel(E_J("rather trippy.","キメキメになった。"));
		else You_feel(E_J("rather %s.","少し%s。"), body_part(LIGHT_HEADED));
...
#endif
		    Sprintf(tmp, "あなたは%sに倒れこんだ", surface(u.ux,u.uy));
		}
		pline("世界が回り出し、%s。", w);
#endif /*JP*/
		flags.soundok = 0;
		nomul(-rnd(10));
		nomovemsg = E_J("You are conscious again.","あなたは意識を取り戻した。"); /* see unconscious() */
		afternmv = Hear_again;
		return(1);
	}
	return(0);
	...
	register struct obj *otmp;
{
	...
	if (victual.tainted) {
		...
		pline(E_J("Ulch - that %s was tainted%s!",
			  "ウェ… この%sは汚染されてい%s！"),
		      mons[mnum].mlet == S_FUNGUS ? E_J("fungoid vegetation","菌茸類") :
		      !vegetarian(&mons[mnum]) ? E_J("meat","肉") : E_J("protoplasm","原形質"),
		      cannibal ? E_J(" cannibal","て、その上共食いだ") : E_J("","る"));
		if (Sick_resistance) {
			pline(E_J("It doesn't seem at all sickening, though...",
				  "まあ、実のところそれほど酷いものでもないようだが…。"));
		} else {
			char buf[BUFSZ];
			long sick_time;
			...
			if (!uniq)
			    Sprintf(buf, E_J("rotted %s","腐った%s"), corpse_xname(otmp,TRUE));
			else
#ifndef JP
			    Sprintf(buf, "%s%s rotted corpse",
				    ...
				    s_suffix(mons[mnum].mname));
#else
			    Sprintf(buf, "%sの腐った死体", JMONNAM(mnum));
#endif /*JP*/
		...
		You(E_J("have a very bad case of stomach acid.",
			"重度の胃酸過多症となった。")); /* not body_part() */
		losehp(rnd(15), E_J("acidic corpse","強酸性の死体を食べて"), KILLED_BY_AN);
	} else if (victual.poison) {
		pline(E_J("Ecch - that must have been poisonous!",
			  "ゲェ… 有毒だったに違いない！"));
		if(!Poison_resistance) {
			losestr(rnd(4));
			losehp(rnd(15), E_J("poisonous corpse","有毒な死体を食べて"), KILLED_BY_AN);
		} else	You(E_J("seem unaffected by the poison.",
				"毒の影響を受けないようだ。"));
	/* now any corpse left too long will make you mildly ill */
	} else if (victual.rotsick) {
		You_feel(E_J("%ssick.","%s気分が悪くなった。"), (Sick) ? E_J("very ","とても") : "");
		losehp(rnd(8), E_J("cadaver","腐乱死体を食べて"), KILLED_BY_AN);
	}
	...
	register struct obj *otmp;
{
	...
	Sprintf(msgbuf, E_J("eating %s","%sを食べるの"), food_xname(otmp, TRUE));
	...
	switch(otmp->otyp) {
		...
		if(u.uhunger <= 200)
		    pline(Hallucination ? E_J("Oh wow, like, superior, man!",
			    "うーむ、まったりとして、それでいてしつこくない！") :
			  E_J("That food really hit the spot!","この食事はまさに申し分ない！"));
#ifndef JP
		...
		if (carnivorous(youmonst.data) && !humanoid(youmonst.data))
		    ...
		    pline(Hallucination ? E_J("Tastes great! Less filling!","うまい！ もっとくれ！") :
			  E_J("Mmm, tripe... not bad!","んん、モツか…悪くない！"));
		else {
		    pline(E_J("Yak - dog food!","うげ…ドッグフードだ！"));
		...
		if (otmp->otyp==SLIME_MOLD && !otmp->cursed
		    ...
		    pline(E_J("My, that was a %s %s!","おや、なんて%s%sだ！"),
			  Hallucination ? E_J("primo","極上の") : E_J("yummy","おいしい"),
			  singular(otmp, xname));
		else
#ifdef UNIX
...
/* This is based on an old Usenet joke, a fake a.out manual page */
		...
		if (otmp->otyp == EGG && stale_egg(otmp)) {
		    pline(E_J("Ugh.  Rotten egg.",
			      "うぐ。腐った卵だ。"));	/* perhaps others like it */
		    make_vomiting(Vomiting+d(10,4), TRUE);
		} else
 give_feedback:
		    pline(E_J("This %s is %s","この%sは%s"), singular(otmp, xname),
		      otmp->cursed ? (Hallucination ? E_J("grody!","イケてない！") : E_J("terrible!","ひどい味だ！")) :
		      (otmp->otyp == CRAM_RATION
		      || otmp->otyp == K_RATION
		      || otmp->otyp == C_RATION)
		      ? E_J("bland.","味気ない。") :
		      Hallucination ? E_J("gnarly!","イケてる！") : E_J("delicious!","おいしい！"));
		break;
	}
...
struct obj *otmp;
{
	pline(E_J("Magic spreads through your body as you digest the %s.",
		  "%sが消化されるとともに、魔力があなたの体に染み渡った。"),
	    otmp->oclass == RING_CLASS ? E_J("ring","指輪") : E_J("amulet","魔除け"));
}
...
struct obj *otmp;
{
	...
	if (!rn2(otmp->oclass == RING_CLASS ? 3 : 5)) {
	  switch (otmp->otyp) {
		...
		switch (typ) {
		  ...
		  case RIN_INVISIBILITY:
			...
			Your(E_J("body takes on a %s transparency...",
				 "体は%s透明性を帯びた…。"),
				Hallucination ? E_J("normal","普通に") : E_J("strange","不思議な"));
			makeknown(typ);
		    }
		    break;
		...
		makeknown(typ);
		change_sex();
		You(E_J("are suddenly very %s!","突然とても%sらしくなった！"),
		    flags.female ? E_J("feminine","女") : E_J("masculine","男"));
		flags.botl = 1;
		break;
...
eatspecial() /* called after eating non-food */
{
	...
	set_occupation(eatfood, E_J("eating non-food","食べるの"), 0);
	...
	{
		pline(E_J("Yabba-dabba delicious!","ヤッバダッバうまい！"));
		exercise(A_CON, TRUE);
	}
...
register struct obj *otmp;
{
	if (otmp->oclass == FOOD_CLASS) return E_J("food","食べ物");
...
register struct obj *otmp;
{
	switch(otmp->otyp) {
		...
		if (Upolyd) {
			...
			killer_format = KILLED_BY_AN;
			killer = E_J("rotten lump of royal jelly","腐ったロイヤルゼリーの毒にあたり");
			done(POISONING);
		    }
		}
		...
		if (touch_petrifies(&mons[otmp->corpsenm])) {
			...
			if (!Stoned) Stoned = 5;
			killer_format = KILLED_BY_AN;
			Sprintf(killer_buf, E_J("%s egg","%sの卵を食べて"),
...
struct obj *otmp;
{
	...
	Strcpy(it_or_they, (otmp->quan == 1L) ? "it" : "they");
	Sprintf(eat_it_anyway, "それでも食べますか？");
#endif /*JP*/
	...
	if (stoneorslime) {
		...
		Sprintf(buf, "%s like %s could be something very dangerous! %s",
			foodsmell, it_or_they, eat_it_anyway);
#else
		Sprintf(buf, "%sは非常に危険な臭いを放っている！ %s",
			foodsmell, eat_it_anyway);
#endif /*JP*/
		if (yn_function(buf, ynchars,'n')=='n') return 1;
		else return 2;
	...
	if (victual.poison && !Poison_resistance) {
		...
		Sprintf(buf, "%s like %s might be poisonous! %s",
			foodsmell, it_or_they, eat_it_anyway);
#else
		Sprintf(buf, "%sは毒を持っていそうな臭いがする！ %s",
			foodsmell, eat_it_anyway);
#endif /*JP*/
		if (yn_function(buf,ynchars,'n')=='n') return 1;
		else return 2;
	...
	if (fcnd >= 2 && !u.uconduct.unvegetarian && Role_if(PM_MONK)) {
		...
		Sprintf(buf, "%s unhealthy. %s",
			foodsmell, eat_it_anyway);
#else
		Sprintf(buf, "%sからは健康に悪そうな臭いがする。 %s",
			foodsmell, eat_it_anyway);
#endif /*JP*/
		if (yn_function(buf,ynchars,'n')=='n') return 1;
		else return 2;
	...
	if (cadaver && acidic(&mons[mnum]) && !Acid_resistance) {
		...
		Sprintf(buf, "%s rather acidic. %s",
			foodsmell, eat_it_anyway);
#else
		Sprintf(buf, "%sからはかなり酸っぱい臭いがする。 %s",
			foodsmell, eat_it_anyway);
#endif /*JP*/
		if (yn_function(buf,ynchars,'n')=='n') return 1;
		else return 2;
	...
	if (Upolyd && u.umonnum == PM_RUST_MONSTER &&
		...
		Sprintf(buf, "%s disgusting to you right now. %s",
			foodsmell, eat_it_anyway);
#else
		Sprintf(buf, "%sは吐き気を催す臭いを放っている。 %s",
			foodsmell, eat_it_anyway);
#endif /*JP*/
		if (yn_function(buf,ynchars,'n')=='n') return 1;
		else return 2;
	...
	if (!u.uconduct.unvegan && fcnd >= 1) {
		...
		Sprintf(buf, "%s foul and unfamiliar to you. %s",
			foodsmell, eat_it_anyway);
#else
		Sprintf(buf, "%sからは食べたことのない、穢れた臭いがする。 %s",
			foodsmell, eat_it_anyway);
#endif /*JP*/
		if (yn_function(buf,ynchars,'n')=='n') return 1;
		else return 2;
	...
	if (!u.uconduct.unvegetarian && fcnd >= 2) {
		...
		Sprintf(buf, "%s unfamiliar to you. %s",
			foodsmell, eat_it_anyway);
#else
		Sprintf(buf, "%sからは食べたことのない臭いがする。 %s",
			foodsmell, eat_it_anyway);
#endif /*JP*/
		if (yn_function(buf,ynchars,'n')=='n') return 1;
		else return 2;
...
doeat()		/* generic "eat" command funtion (see cmd.c) */
{
...
#ifdef JP
	static const struct getobj_words eatw = { 0, 0, "食べる", "食べ" };
#endif /*JP*/

	if (Strangled) {
		pline(E_J("If you can't breathe air, how can you consume solids?",
			  "空気も吸えないのに、どうやって物を飲み込むんだい？"));
		return 0;
	}
	if (getobj_override) {
	 ...
	 */
	if (!is_edible(otmp)) {
	    You(E_J("cannot eat that!","そんなものを食べられない！"));
	    return 0;
	...
	if (otmp->otyp == RIN_SLOW_DIGESTION) {
		pline(E_J("This ring is indigestible!","この指輪は消化できない！"));
...
//	    if (otmp->oclass == WEAPON_CLASS && otmp->opoisoned) {
...
#else
		pline("ぐぇ… この%sは有毒だったに違いない！", foodword(otmp));
#endif /*JP*/
		if(!Poison_resistance) {
		    losestr(rnd(4));
		    losehp(rnd(15), xname(otmp), KILLED_BY_AN);
		} else
		    You(E_J("seem unaffected by the poison.","毒の影響を受けないようだ。"));
	    } else if (!otmp->cursed)
		pline(E_J("This %s is delicious!","この%sはうまい！"),
	 ...
	 */
	    ...
	    You(E_J("resume your meal.","食事を再開した。"));
	...
	if(u.uhunger >= 2000) {
		...
		if (!victual.eating || (victual.eating && !victual.fullwarn)) {
		    pline(E_J("You're having a hard time getting all of it down.",
			      "あなたは全てを飲み込むのにとても苦労している。"));
		    nomovemsg = E_J("You're finally finished.","あなたはようやく食べ終えた。");
		    if (!victual.eating)
			multi = -2;
		    else {
			...
			if (victual.canchoke && victual.reqtime > 1) {
			    ...
			    if (yn_function(E_J("Stop eating?","食べるのを中断しますか？"),ynchars,'y')=='y') {
				reset_eat();
				nomovemsg = (char *)0;
			    }
			}
...
sync_hunger()
{
	...
	if(is_fainted()) {
		...
		nomovemsg = E_J("You regain consciousness.","あなたは意識を取り戻した。"); /* see unconscious() */
		afternmv = unfaint;
	}
}
#endif
...
boolean incr;
{
	...
	if(newhs == FAINTING) {
		...
		if(u.uhs <= WEAK || rn2(20-u.uhunger/10) >= 19) {
			if(!is_fainted() && multi >= 0 /* %% */) {
				...
				stop_occupation();
				You(E_J("faint from lack of food.","空腹のあまり気絶した。"));
				flags.soundok = 0;
				nomul(-10+(u.uhunger/10));
				nomovemsg = E_J("You regain consciousness.","あなたは意識を取り戻した。"); /* see unconscious() */
				afternmv = unfaint;
		...
		if(u.uhunger < -(int)(200 + 20*ACURR(A_CON))) {
			...
			flags.botl = 1;
			bot();
			You(E_J("die from starvation.","餓死した。"));
			killer_format = KILLED_BY;
			killer = E_J("starvation","飢えて");
			done(STARVING);
	...
	if(newhs != u.uhs) {
		...
		case HUNGRY:
			if (Hallucination) {
			    ...
			    You((!incr) ? E_J("only feel hungry now.",
					      "衰弱するほど空腹ではなくなった。") :
				  (u.uhunger < 145) ? E_J("feel hungry.","空腹を感じている。") :
	  ...
	  "腹ペコレベルの上昇で、あなたの駆動性能に影響が出始めた。"));
			    ...
			    You((!incr) ? E_J("feel weak now.","まだ衰弱している。") :
				  (u.uhunger < 45) ? E_J("feel weak.","衰弱している。") :
		...
		if ((Upolyd ? u.mh : u.uhp) < 1) {
			...
			killer = E_J("exhaustion","空腹による衰弱で");
			done(STARVING);
			return;
		}
	}
	...
	int corpsecheck; /* 0, no check, 1, corpses, 2, tinnable corpses */
{
	...
	char qbuf[QBUFSZ];
	char c;
	boolean feeding = (E_J(!strcmp(verb, "eat"),!strncmp(verb->suru, "食べる", 6)));
	...
	if (feeding && metallivorous(youmonst.data)) {
		...
		Sprintf(qbuf, E_J("There is a bear trap here (%s); eat it?",
				  "ここには%sトラバサミがある。食べますか？"),
			(u.utrap && u.utraptype == TT_BEARTRAP) ?
				E_J("holding you","あなたを捕らえている") : E_J("armed","仕掛けられた"));
		...
		if (gold->quan == 1L)
		    ...
		    Sprintf(qbuf, "There are %ld gold pieces here; eat them?",
			    gold->quan);
#else
		Sprintf(qbuf, "ここには%ld枚の金貨がある。食べますか？",
			gold->quan);
#endif /*JP*/
		if ((c = yn_function(qbuf, ynqchars, 'n')) == 'y') {
		    return gold;
	...
	for (otmp = level.objects[u.ux][u.uy]; otmp; otmp = otmp->nexthere) {
		...
		(otmp->otyp==CORPSE && (corpsecheck == 1 || tinnable(otmp))) :
			...
			Sprintf(qbuf, "There %s %s here; %s %s?",
				...
				(otmp->quan == 1L) ? "it" : "one");
#else
			Sprintf(qbuf, "ここには%sがある。%s%sますか？",
				doname(otmp),
				(otmp->quan == 1L) ? "" : "一つ", verb->shimasu);
#endif /*JP*/
			if((c = yn_function(qbuf,ynqchars,'n')) == 'y')
				return(otmp);
			else if(c == 'q')
