...
STATIC_DCL int FDECL(check_selected_vaults, (int,int));
static uchar selected_vaults[(NUM_VAULTS+7)>>3];
    ...
    xchar lx, ly, hx, hy;
{
    return((boolean)(occupied(x, y) ||
	   ...
	   !((levl[x][y].typ == CORR && level.flags.is_maze_lev) ||
	       IS_FLOOR(levl[x][y].typ) || levl[x][y].typ == AIR)));
}
	...
	create_secret_door(croom, W_ANY);
	   ...
	   /* it's a ghost town, get rid of shopkeepers */
	    for(mtmp = fmon; mtmp; mtmp = mtmp2) {
			...
			mongone(mtmp);
			/* place shopkeeper's ghost instead... */
			if (!(mtmp = makemon(&mons[PM_GHOST], x, y, NO_MM_FLAGS)))
				continue;
			mtmp->msleeping = 1;
...
#undef y_maze_min
	...
	for(x = rn1(5,7); x; x--) {
		mazexy(&mm);
		(void) makemon((struct permonst *) 0, mm.x, mm.y, MM_MONSTEEDOK/*NO_MM_FLAGS*/);
	}
	for(x = rn1(6,7); x; x--) {
		mazexy(&mm);
...
water_friction()
{
	...
	if (eff) pline(E_J("Water turbulence affects your movements.",
			   "…‚Ì’ïR‚ª‚ ‚È‚½‚Ì“®‚«‚É‰e‹¿‚ð—^‚¦‚½B"));
}
	...
	if (is_lava(x,y))
	  ...
	  !Is_medusa_level(&u.uz) && !Is_waterlevel(&u.uz) && !Is_juiblex_level(&u.uz)) ||
	   (ltyp == DRAWBRIDGE_UP && (levl[x][y].drawbridgemask & DB_UNDER) == DB_MOAT))
		...
		return E_J("pool of water","…ê");
	else return E_J("water","…");
}
 ...
 *****************************************************************/
STATIC_OVL int
check_selected_vaults(index, set)
int index, set;
{
	...
	i = index >> 3;
	m = mask[index & 7];
	r = !!(selected_vaults[i] & m);
	if (set) selected_vaults[i] |= m;
	return r;
}
...
{
	char vnam[BUFSZ];
	int candidate[NUM_VAULTS];
	int i, n, z;
	if (!fn) {
	    /* prepare candidates */
	    z = depth(&u.uz);
	    for (i=0, n=0; i<NUM_VAULTS; i++) {
		if (z >= rndvaults[i].minlev &&
		    z <= rndvaults[i].maxlev &&
		    !check_selected_vaults(i, 0) &&
#ifdef WIZARD
		    rn2(wizard ? 30 : 1000) < rndvaults[i].rarity
#else
		    rn2(1000) < rndvaults[i].rarity
#endif /*JP*/
		   ) {
		    candidate[n++] = i;
		}
	    ...
	    if (!n) return;
	    i = candidate[rn2(n)];
	    check_selected_vaults(i, 1);
	    /* load the preset vault */
	    Sprintf(vnam, "rndv-%03d%s", i, LEV_EXT);
	    load_preset_vault(vnam);
	} else {
	    load_preset_vault(fn);
	}
	if(lev_message) {
	    free((genericptr_t)lev_message);
	    lev_message = 0;
...
{
	int i;
	for (i=0; i<sizeof(selected_vaults); i++) {
	    selected_vaults[i] = 0;
	}
}

void
save_rndvault(fd, mode)
int fd, mode;
{
	int s;
	s = sizeof(selected_vaults);
	if (perform_bwrite(mode)) {
	    bwrite(fd, (genericptr_t) &s, sizeof s);
	    bwrite(fd, (genericptr_t)selected_vaults, s);
	}
}
...
{
	int i, s;
	mread(fd, (genericptr_t) &s, sizeof s);
	if (s != sizeof(selected_vaults)) {
	    impossible("Incompatible RNDVAULT configuration.");
	    for (i=0; i<sizeof(selected_vaults); i++)
		selected_vaults[i] = 0xff;
	} else {
	    mread(fd, (genericptr_t) selected_vaults, sizeof selected_vaults);
	}
}
