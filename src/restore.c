...
#endif /* GOLDOBJ */
	    if (otmp->in_use) {
		if (!quietly) pline(E_J("Finishing off %s...","%sを処分します…。"), xname(otmp));
		useup(otmp);
	    }
	}
}
...
boolean ghostly;
{
	...
	/* relink id to ptr */
	for (mtmp = first; mtmp; mtmp=mtmp->nmon) {
	    unsigned mid;
	    mid = (unsigned)(mtmp->mlink);
	    if (mid) mtmp->mlink = id_to_monptr(mid, first, ghostly);
	}
...
unsigned int *stuckid, *steedid;	/* STEED */
{
	...
	if (uid != getuid()) {		/* strange ... */
	    ...
	    pline(E_J("Saved game was not yours.",
		      "あなたのセーブファイルではありません。"));
#ifdef WIZARD
	    if (!wizard)
#endif
...
# endif
			pline(E_J("Be seeing you...","またお会いしましょう…。"));
...
# endif
	...
	You("return to level %d in %s%s.",
		depth(&u.uz), E_J(dungeons[u.uz.dnum].dname, jdgnnam(u.uz.dnum)),
...
#else
	pline("おかしい、このマップは私の憶えているものと違う。");
	pline("誰かがイカサマを試みているようだ…。");
	pline("このゲームは無効だ。");
#endif /*JP*/
	killer = reason;
	done(TRICKED);
...
/* CHECK:  This may prevent restoration */
	...
	if ((pid && pid != hpid) || (lev && dlvl != lev)) {
	    ...
	    if (pid && pid != hpid)
		Sprintf(trickbuf, E_J("PID (%d) doesn't match saved PID (%d)!",
				      "PID(%d)がセーブされたPID(%d)と一致しない！"),
			hpid, pid);
	    else
		Sprintf(trickbuf, E_J("This is level %d, not %d!",
				      "これはレベル%dだ、%dじゃない！"), dlvl, lev);
#ifdef WIZARD
	...
	if (u.uz.dlevel) {
		...
		if (ghostly) {
			...
			setmpeaceful(mtmp, mtmp->mpeaceful);
...
#ifdef MONSTEED
	    /* don't place monsteeds on map */
	    if (!is_mridden(mtmp) && !is_swallowed(mtmp))
#endif /*MONSTEED*/
	...
	restore_cloud(fd, (ghostly) ? 0 : monstermoves - omoves);
...
/* relink id to monst ptr */
/* limitation: all linked monsters should be in the same chain */
STATIC_OVL struct monst *
id_to_monptr(mid, mchn, ghostly)
unsigned mid;
struct monst *mchn;
boolean ghostly;
{
	struct monst *mtmp;
	if (ghostly)
	    if (!lookup_id_mapping(mid, &mid)) {
		impossible("id_to_monptr: no id in monchain?");
		return 0;
	    }
	for (mtmp = mchn; mtmp; mtmp = mtmp->nmon)
	    if (mtmp->m_id == mid) break;
	if (!mtmp) impossible("id_to_monptr: no monptr is found?");
	return mtmp;
}
