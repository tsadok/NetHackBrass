...
register struct monst *mtmp;
{
	...
	if (!is_clinging(mtmp) && !likes_lava(mtmp->data)) {
		...
		if (--mtmp->mhp < 1) {
		    ...
		    pline(E_J("%s burns slightly.","炎熱に耐えている。"), Monnam(mtmp));
	    }
	    if (mtmp->mhp > 0) {
...
mcalcdistress()
{
	...
	if (mtmp->cham &&
	    ...
	    (void) newcham(mtmp, (struct permonst *)0, FALSE,
			   (mtmp->data == &mons[PM_DOPPELGANGER] ||
...
#ifdef MONSTEED
	if (!is_mridden(mon)) /* cancel remove_monster() for msteed */
#endif
...
int  typ, fatal;
{
	...
	boolean is_blast = !strcmp(string, "blast");

	if (thrown_weapon) fatal = -fatal;
	if(!is_blast && !thrown_weapon) {
...
boolean msg;		/* "The oldmon turns into a newmon!" */
{
	...
	/* doppelganger */
	copyu = ((mtmp->data == &mons[PM_DOPPELGANGER] ||
		  mtmp->cham == CHAM_DOPPELGANGER) &&
...
/* and the same for maximum hit points */
	...
	if (msg) {
	    ...
	    pline(E_J("%s turns into %s!","%sは%sに変化した！"), oldname,
		  mdat == &mons[PM_GREEN_SLIME] ? E_J("slime","スライム") :
		  copyu ? E_J("you","あなた") :
