...
struct entity *etmp;
{
	return(is_u(etmp)? E_J("you","Ç†Ç»ÇΩ") : mon_nam(etmp->emon));
}
...
struct entity *etmp;
{
	return(is_u(etmp)? E_J("You","Ç†Ç»ÇΩ") : Monnam(etmp->emon));
}
#endif /* D_DEBUG */
	...
	if (is_u(etmp)) {
		if (how == DROWNING) {
			...
			if (!killer) {
			    ...
			    killer = E_J("falling drawbridge","ç~ÇËÇƒÇ´ÇΩíµÇÀã¥Ç…âüÇµí◊Ç≥ÇÍÇƒ");
			}
			done(how);
			/* So, you didn't die */
			if (!e_survives_at(etmp, etmp->ex, etmp->ey)) {
				...
				pline(E_J("A %s force teleports you away...",
					  "%sóÕÇ™Ç†Ç»ÇΩÇâ^Ç—ãéÇ¡ÇΩÅcÅB"),
				      Hallucination ? E_J("normal","Ç†ÇËÇ´ÇΩÇËÇ»") : E_J("strange","ïsévãcÇ»"));
...
struct entity *etmp;
{
	...
	if (automiss(etmp) && e_survives_at(etmp, oldx, oldy)) {
		if (e_inview && (at_portcullis || IS_DRAWBRIDGE(crm->typ)))
			pline_The(E_J("%s passes through %s!",
				      "%sÇÕ%sÇí ÇËî≤ÇØÇΩÅI"),
			      at_portcullis ? E_J("portcullis","óéÇ∆Çµäiéq") : E_J("drawbridge","íµÇÀã¥"),
			      e_nam(etmp));
	...
	if (e_missed(etmp, FALSE)) {
		if (at_portcullis)
			pline_The(E_J("portcullis misses %s!","óéÇ∆ÇµäiéqÇÕ%sÇ…ìñÇΩÇÁÇ»Ç©Ç¡ÇΩÅI"),
			      e_nam(etmp));
#ifdef D_DEBUG
		else
			pline_The(E_J("drawbridge misses %s!","íµÇÀã¥ÇÕ%sÇ…ìñÇΩÇÁÇ»Ç©Ç¡ÇΩÅI"),
			      e_nam(etmp));
#endif
	...
	if (must_jump) {
		...
		if (e_jumps(etmp)) {
			...
			pline(E_J("%s crushed by the falling portcullis!",
				  "%sÇÕç~â∫ÇµÇΩóéÇ∆ÇµäiéqÇ…âüÇµí◊Ç≥ÇÍÇΩÅI"),
			      E_phrase(etmp, "are"));
		    else if (flags.soundok)
			You_hear(E_J("a crushing sound.","âΩÇ©Ç™âüÇµí◊Ç≥ÇÍÇÈâπÇ"));
 ...
 */
	...
	if (is_db_wall(etmp->ex, etmp->ey)) {
		...
		if (e_inview) {
			if (is_u(etmp)) {
				You(E_J("tumble towards the closed portcullis!",
					"ï¬Ç∂ÇΩóéÇ∆ÇµäiéqÇ…å¸Ç©Ç¡Çƒì]Ç™Ç¡ÇƒÇ¢Ç¡ÇΩÅI"));
				if (automiss(etmp))
					You(E_J("pass through it!","äiéqÇí ÇËî≤ÇØÇΩÅI"));
				else
					pline_The(E_J("drawbridge closes in...",
						      "íµÇÀã¥Ç™îóÇ¡ÇƒÇ≠ÇÈÅcÅB"));
			} else
				pline(E_J("%s behind the drawbridge.",
					  "%sÇÕíµÇÀã¥ÇÃîwå„Ç…è¡Ç¶ÇΩÅB"),
				      E_phrase(etmp, "disappear"));
		}
		if (!e_survives_at(etmp, etmp->ex, etmp->ey)) {
			killer_format = KILLED_BY_AN;
			killer = E_J("closing drawbridge","ï¬Ç∂ÇÈíµÇÀã¥Ç…âüÇµí◊Ç≥ÇÍÇƒ");
		...
		if (is_pool(etmp->ex, etmp->ey) && !e_inview)
			if (flags.soundok)
				You_hear(E_J("a splash.","êÖâπÇ"));
		if (e_survives_at(etmp, etmp->ex, etmp->ey)) {
			if (e_inview && !is_flying(etmp->emon) &&
			    !is_floating(etmp->emon))
				pline(E_J("%s from the bridge.","%sÇÕã¥Ç©ÇÁóéÇøÇΩÅB"),
				      E_phrase(etmp, "fall"));
			return;
		}
#ifdef D_DEBUG
		...
		if (is_pool(etmp->ex, etmp->ey) || is_lava(etmp->ex, etmp->ey))
			...
			if (Hallucination)
			    pline(E_J("%s the %s and disappears.",
				      "%sÇÕ%sÇà˘ÇÒÇ≈è¡Ç¶ÇΩÅB"),
				  E_phrase(etmp, "drink"),
				  lava ? E_J("lava","É}ÉOÉ}") : E_J("moat","ñxÇÃêÖ"));
			else
			    pline(E_J("%s into the %s.","%sÇÕ%sÇ…óéÇøÇΩÅB"),
				  E_phrase(etmp, "fall"),
				  lava ? E_J("lava","ónä‚") : E_J("moat","ñx"));
		    }
		killer_format = NO_KILLER_PREFIX;
		killer = E_J("fell from a drawbridge","íµÇÀã¥Ç©ÇÁóéÇøÇƒéÄÇÒÇæ");
...
int x,y;
{
	...
	if (cansee(x,y) || cansee(x2,y2))
		You(E_J("see a drawbridge %s up!","íµÇÀã¥Ç™è„Ç™%sÇÈÇÃÇå©ÇΩÅI"),
		    (((u.ux == x || u.uy == y) && !Underwater) ||
		     distu(x2,y2) < distu(x,y)) ? E_J("coming","Ç¡ÇƒÇ≠") : E_J("going",""));
	...
	if(OBJ_AT(x,y) && flags.soundok)
	    You_hear(E_J("smashing and crushing.","à≥í◊Ç∆ï≤ç”ÇÃâπÇ"));
...
int x,y;
{
	...
	get_wall_for_db(&x2,&y2);
	if (cansee(x,y) || cansee(x2,y2))
		You(E_J("see a drawbridge %s down!","íµÇÀã¥Ç™â∫ÇËÇƒ%sÇÃÇå©ÇΩÅI"),
		    (distu(x2,y2) < distu(x,y)) ? E_J("going","Ç‰Ç≠") : E_J("coming","Ç≠ÇÈ"));
...
int x,y;
{
	...
	if (db_u == DB_MOAT || db_u == DB_LAVA || db_u == DB_BOG) {
		...
		int where = (db_u == DB_LAVA) ? 0 :
			    (db_u == DB_MOAT) ? 1 : 2;
		static char *wstr[3] = { E_J("lava","ónä‚"), E_J("moat","ñx"), E_J("swamp","è¿") };
		if (lev1->typ == DRAWBRIDGE_UP) {
			if (cansee(x2,y2))
			    pline_The(E_J("portcullis of the drawbridge falls into the %s!",
					  "íµÇÀã¥ÇÃóéÇ∆ÇµäiéqÇÕ%sÇ…óéâ∫ÇµÇΩÅI"), wstr[where]);
			else if (flags.soundok)
				You_hear(E_J("a loud *SPLASH*!","!åÉÇµÇ¢êÖâπÇ"));
		} else {
			if (cansee(x,y))
			    pline_The(E_J("drawbridge collapses into the %s!",
					  "íµÇÀã¥ÇÕ%sÇÃíÜÇ…ïˆÇÍóéÇøÇΩÅI"), wstr[where]);
			else if (flags.soundok)
				You_hear(E_J("a loud *SPLASH*!","!åÉÇµÇ¢êÖâπÇ"));
		}
		...
		if (cansee(x,y))
			pline_The(E_J("drawbridge disintegrates!","íµÇÀã¥ÇÕï™âÇ≥ÇÍÇΩÅI"));
		else
			You_hear(E_J("a loud *CRASH*!","!ã≠óÛÇ»îjâÛâπÇ"));
		lev1->typ =
		...
		if (!automiss(etmp2)) {
			if (e_inview)
				pline(E_J("%s blown apart by flying debris.",
					  "îÚéUÇµÇΩîjï–Ç™%sÇ…åÉìÀÇµÅAÇŒÇÁÇŒÇÁÇ…êÅÇ´îÚÇŒÇµÇΩÅB"),
				      E_phrase(etmp2, "are"));
			killer_format = KILLED_BY_AN;
			killer = E_J("exploding drawbridge","íµÇÀã¥ÇÃ‡yóÙÇ≈");
		...
		if (e_missed(etmp1, TRUE)) {
			...
			if (e_inview) {
				...
				pline(E_J("%s into some heavy metal!",
					  "%sÇÕÉwÉrÅ[ÅEÉÅÉ^ÉãÇÃíÜÇ…ì¸Ç¡ÇΩÅI"),
				      E_phrase(etmp1, "get"));
			    else
				pline(E_J("%s hit by a huge chunk of metal!",
					  "ãêëÂÇ»ìSâÚÇ™%sÇ…íºåÇÇµÇΩÅI"),
				      E_phrase(etmp1, "are"));
			} else {
			    if (flags.soundok && !is_u(etmp1) && !is_pool(x,y))
				You_hear(E_J("a crushing sound.","âΩÇ©Ç™í◊ÇÍÇÈâπÇ"));
#ifdef D_DEBUG
			    else
			...
			killer_format = KILLED_BY_AN;
			killer = E_J("collapsing drawbridge","ïˆÇÍóéÇøÇΩíµÇÀã¥Ç…í◊Ç≥ÇÍÇƒ");
