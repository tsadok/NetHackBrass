...
STATIC_DCL void FDECL(objdesc_by_invlet, (char));
...
#else
	    pline(E_J("Not carrying anything.","あなたは何も所持していない。"));
#endif
	    return 0;
	}
...
char *buf;
{
	...
	if (IS_DOOR(ltyp)) {
	    ...
	    Sprintf(altbuf, E_J("altar to %s (%s)","%sの祭壇 (%s)"), a_gname(),
...
doprgold()
{
	...
	if(!umoney)
	    ...
	    Your(E_J("wallet contains %ld %s.",
		     "財布には%ld %s入っている。"), umoney, currency(umoney));
#endif
...
#else
	Sprintf(tmp,"%sの%s:",
		(In_endgame(&u.uz) && mon->data == &mons[PM_HIGH_PRIEST]) ?
