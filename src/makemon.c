...
{
	struct monst *mtmp;
	int mndx, mcham, ct, mitem, xlth, mstd;
	boolean anymon = (!ptr);
...
#else
		    pline("%s‚ª¬’·‚µ‚Ä%s‚É‚È‚Á‚½‚Æ‚½‚ñA%s‚Í%sI", mon_nam(mtmp),
			JMONNAM(monsndx(ptr)), mhe(mtmp),
			nonliving(ptr) ? "“|‚ê‚½" : "Ž€‚ñ‚¾");
#endif /*JP*/
