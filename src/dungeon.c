...
int		/* which part of hell are you in? */
In_which_hell(lev)
d_level	*lev;
{
	int d;
	if (!In_hell(lev)) return -1;
	d = depth(lev);
	/* this order depends on dungeon.def... */
	if (Is_valley(lev)) return INHELL_MAZE;
	if (On_W_tower_level(lev)) return INHELL_MAZE;
	if (d <= depth(&asmodeus_level)) return INHELL_ASMODEUS;
	if (d <= depth(&juiblex_level)) return INHELL_JUIBLEX;
	if (d <= depth(&baalzebub_level)) return INHELL_BAALZEBUB;
	if (d <= depth(&orcus_level)) return INHELL_ORCUS;
	if (d <= depth(&mlcgate_level)) return INHELL_HELL;
	return INHELL_MAZE;
}
...
recalc_mapseen()
{
    ...
    mptr->flags.sokosolved = In_sokoban(&u.uz) && level.flags.sokoban_rules;
...
# endif
	...
	/* Update lastseentyp with typ if and only if it is in sight or
	 * the hero can feel it on their current location (i.e. not
	 * levitating).  This *should* give the "last known typ" for each
	 * dungeon location.  (At the very least, it's a better assumption
	 * than determining what the player knows from the glyph and the
	 * typ (which is isn't quite enough information in some cases).
	 *
    ...
    for (x = 1; x < COLNO; x++) {
        for (y = 0; y < ROWNO; y++) {
            if (cansee(x, y) || (x == u.ux && y == u.uy && !Levitation)) {
                ...
                if ((mtmp != 0) && mtmp->m_ap_type == M_AP_FURNITURE &&
                    canseemon(mtmp))
...
struct monst *priest; /* currently UNUSED; might be useful someday */
{
...
branch *br;
{
    ...
    boolean closed_portal = (br->end2.dnum == quest_dnum && u.uevent.qexpelled);
...
int rtype;
{
    ...
    case TOOLSHOP:      str = "tool shop";      break;
    case BOOKSHOP:      str = "bookstore";      break;
    case TAILORSHOP:    str = "tailor shop";    break;
