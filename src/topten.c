...
#else
	"死んだ", "を喉に詰まらせた", "を食べて病死した", "死んだ", "溺死した",
	"焼死した", "溶解した", "押し潰された", "石化した",
	"スライムと化した", "殺された", "", "", "", "", ""
#endif /*JP*/
};
...
#define HUP
	...
	if (wizard || discover) {
		...
		topten_print("");
		Sprintf(pbuf,
	  E_J("Since you were in %s mode, the score list will not be checked.",
	      "%sモードでプレイしたため、スコアリストには載りません。"),
		    wizard ? E_J("wizard","Wizard") : E_J("discover","発見"));
		topten_print(pbuf);
	    }
	    goto showwin;
	}
	...
	for(rank = 1; ; ) {
		...
		--occ_cnt <= 0) {
			...
			HUP {
			    char pbuf[BUFSZ];
			    Sprintf(pbuf,
			E_J("You didn't beat your previous score of %ld points.",
			    "あなたは以前の自分のスコア %ldポイントに届かなかった。"),
				    t1->points);
			    topten_print(pbuf);
			    topten_print("");
			}
	...
	if(flg) {	/* rewrite record file */
		...
		if(!done_stopprint) if(rank0 > 0){
		    if(rank0 <= 10)
			topten_print(E_J("You made the top ten list!",
					"あなたは上位10名に名を連ねた！"));
		    else {
			char pbuf[BUFSZ];
#ifndef JP
...
boolean so;
{
	...
	if (!strncmp("escaped", t1->death, 7)) {
		...
		*bp = (t1->deathdnum == astral_level.dnum) ? '\0' : ' ';
	    ...
	    second_line = FALSE;
#else
	if (strlen(t1->death) >= 10 &&
	    !strncmp("逃げ出した", eos(t1->death)-10, 10)) {
	    bp = eos(t1->death) - 10;
	    *bp = 0;
	    Sprintf(eos(linebuf), "%s迷宮から逃げ出した [%d階まで到達]",
		    t1->death, t1->maxlvl);
	    second_line = FALSE;
	} else if (!strncmp("昇天した", t1->death, 8)) {
	    Sprintf(eos(linebuf), "昇天し%s神となった",
		    (t1->plgend[0] == 'F') ? "女" : "");
	    second_line = FALSE;
#endif /*JP*/
	} else {
		...
		const char *arg, *fmt = E_J(" on the Plane of %s","%sの精霊界");

		switch (t1->deathlev) {
		case -5:
			fmt = E_J(" on the %s Plane","%s");
			arg = E_J("Astral","天上界");	break;
		case -4:
			arg = E_J("Water","水");	break;
		case -3:
			arg = E_J("Fire","火");		break;
		case -2:
			arg = E_J("Air","風");		break;
		case -1:
			arg = E_J("Earth","土");	break;
		default:
			arg = "Void";	break;
		}
		Sprintf(eos(linebuf), fmt, arg);
	    } else {
		Sprintf(eos(linebuf), E_J(" in %s","%sの"),
			E_J(dungeons[t1->deathdnum].dname,jdgnnam(t1->deathdnum)));
		if (t1->deathdnum != knox_level.dnum)
		    Sprintf(eos(linebuf), E_J(" on level %d","%d階"), t1->deathlev);
		if (t1->deathlev != t1->maxlvl)
		    Sprintf(eos(linebuf), E_J(" [max %d]"," [%d階まで到達]"), t1->maxlvl);
	    }
#ifdef JP
	    Strcat(linebuf, "にて\t");
#endif /*JP*/
...
#else /*JP*/
	if (second_line)
	    Strcpy(eos(linebuf), t1->death);
	if (t1->hp <= 0) hpbuf[0] = '-', hpbuf[1] = '\0';
	else Sprintf(hpbuf, "%d", t1->hp);
	jprepare_epitaph(linebuf+16, COLNO - 16 - 11);
	for (i=0; i<4; i++) {
	    bp = jget_epitaph(i);
	    if (!*bp) break;
	}
	for (j=0; j<i; j++) {
	    bp = jget_epitaph(j);
	    if (j==0)
		Strcpy(linebuf+16, jget_epitaph(0));
	    else
		Sprintf(linebuf, "%15s %s", "", jget_epitaph(j));
	    if (j >= i-1) break;
	    if (so) {
		bp = eos(linebuf);
