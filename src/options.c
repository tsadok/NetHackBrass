...
//#ifdef AUTOTHRUST
//	{"autothrust", &flags.autothrust, FALSE, SET_IN_GAME},
//#endif
...
# endif
...
#if defined(TERMLIB) && !defined(MAC_GRAPHICS_ENV) && !defined(WIN32_GRAPHICS2)
	...
	{"IBMrogue", &iflags.IBMrogue, TRUE, SET_IN_GAME},
#else
	{"IBMgraphics", (boolean *)0, FALSE, SET_IN_FILE},
	{"IBMrogue", (boolean *)0, TRUE, SET_IN_FILE},
#endif
#ifndef MAC
...
# endif
...
#ifdef PICKUP_THROWN
	{"pickup_thrown", &flags.pickup_thrown, FALSE, SET_IN_GAME},
#else
	{"pickup_thrown", (boolean *)0, FALSE, SET_IN_FILE},
#endif
	...
	{"retry_attr", &iflags.retry_attr, FALSE, SET_IN_FILE},
...
#endif
#ifdef SHOWDMG
	{"showdamage", &flags.showdmg, FALSE, SET_IN_GAME},
#else
	{"showdamage", (boolean *)0, FALSE, SET_IN_FILE},
#endif
#ifdef EXP_ON_BOTL
	{"showexp", &flags.showexp, FALSE, SET_IN_GAME},
	{"showrexp", &flags.showrexp, FALSE, SET_IN_GAME},
#else
	{"showexp", (boolean *)0, FALSE, SET_IN_FILE},
	{"showrexp", (boolean *)0, FALSE, SET_IN_FILE},
#endif
...
static struct Comp_Opt
{
	...
	{ "align",    E_J("your starting alignment (lawful, neutral, or chaotic)",
			  "あなたの開始時の属性 (lawful, neutral, or chaotic)"),
						8, DISP_IN_GAME },
	{ "align_message", E_J("message window alignment",
			       "メッセージウィンドウの配置"), 20, DISP_IN_GAME }, 	/*WC*/
	{ "align_status", E_J("status window alignment",
			      "ステータスウィンドウの配置"), 20, DISP_IN_GAME }, 	/*WC*/
	{ "altkeyhandler", E_J("alternate key handler",
			       "代替のキーボードハンドラ"), 20, DISP_IN_GAME },
	{ "boulder",  E_J("the symbol to use for displaying boulders",
			  "大岩の表示に使うシンボル"),
						1, SET_IN_GAME },
	{ "catname",  E_J("the name of your (first) cat (e.g., catname:Tabby)",
			  "あなたの(開始時の)猫の名前 (例 catname:たま)"),
						PL_PSIZ, DISP_IN_GAME },
	{ "disclose", E_J("the kinds of information to disclose at end of game",
			  "ゲーム終了時に開示する情報の種類"),
						sizeof(flags.end_disclose) * 2,
						SET_IN_GAME },
	{ "dogname",  E_J("the name of your (first) dog (e.g., dogname:Fang)",
			  "あなたの(開始時の)犬の名前 (例 dogname:ポチ)"),
						PL_PSIZ, DISP_IN_GAME },
	{ "dungeon",  E_J("the symbols to use in drawing the dungeon map",
			  "マップの表示に使うシンボルのセット"),
						MAXDCHARS+1, SET_IN_FILE },
	{ "effects",  E_J("the symbols to use in drawing special effects",
			  "特殊効果の表示に使うシンボルのセット"),
						MAXECHARS+1, SET_IN_FILE },
	{ "font_map", E_J("the font to use in the map window",
			  "マップの表示に使うフォント"), 40, DISP_IN_GAME },	/*WC*/
	{ "font_menu", E_J("the font to use in menus",
			   "メニューの表示に使うフォント"), 40, DISP_IN_GAME },		/*WC*/
	{ "font_message", E_J("the font to use in the message window",
			      "メッセージの表示に使うフォント"),
						40, DISP_IN_GAME },		/*WC*/
	{ "font_size_map", E_J("the size of the map font",
			       "マップのフォントサイズ"), 20, DISP_IN_GAME },	/*WC*/
	{ "font_size_menu", E_J("the size of the menu font",
				"メニューのフォントサイズ"), 20, DISP_IN_GAME },	/*WC*/
	{ "font_size_message", E_J("the size of the message font",
				   "メッセージのフォントサイズ"), 20, DISP_IN_GAME },	/*WC*/
	{ "font_size_status", E_J("the size of the status font",
				  "ステータスのフォントサイズ"), 20, DISP_IN_GAME },	/*WC*/
	{ "font_size_text", E_J("the size of the text font",
				"テキストのフォントサイズ"), 20, DISP_IN_GAME },	/*WC*/
	{ "font_status", E_J("the font to use in status window",
			     "ステータスの表示に使うフォント"), 40, DISP_IN_GAME }, /*WC*/
	{ "font_text", E_J("the font to use in text windows",
			   "テキストの表示に使うフォント"), 40, DISP_IN_GAME },	/*WC*/
	{ "fruit",    E_J("the name of a fruit you enjoy eating",
			  "好きな果物の名前"),
						PL_FSIZ, SET_IN_GAME },
	{ "gender",   E_J("your starting gender (male or female)",
			  "あなたの開始時の性別 (male or female)"),
						8, DISP_IN_GAME },
	{ "horsename", E_J("the name of your (first) horse (e.g., horsename:Silver)",
			   "あなたの(開始時の)馬の名前 (例 horsename:シルバー)"),
						PL_PSIZ, DISP_IN_GAME },
	{ "map_mode", E_J("map display mode under Windows",
			  "Windows環境でのマップ表示の方法"), 20, DISP_IN_GAME },	/*WC*/
    { "menucolor", E_J("set menu colors",
              "メニューの色イネーブル"), PL_PSIZ, SET_IN_FILE },
	{ "menustyle", E_J("user interface for object selection",
			   "品物の選択のためのユーザーインタフェイス"),
						MENUTYPELEN, SET_IN_GAME },
	{ "menu_deselect_all", E_J("deselect all items in a menu",
				   "メニューの全アイテムの選択を解除"), 4, SET_IN_FILE },
	{ "menu_deselect_page", E_J("deselect all items on this page of a menu",
				    "メニューの表示中の全アイテムの選択を解除"),
						4, SET_IN_FILE },
	{ "menu_first_page", E_J("jump to the first page in a menu",
				 "メニューの最初のページに移動"),
						4, SET_IN_FILE },
	{ "menu_headings", E_J("bold, inverse, or underline headings",
			       "見出しを bold, inverse, underline で表示"), 9, SET_IN_GAME },
	{ "menu_invert_all", E_J("invert all items in a menu",
				 "メニューの全アイテムの選択状態を反転"), 4, SET_IN_FILE },
	{ "menu_invert_page", E_J("invert all items on this page of a menu",
				  "メニューの表示中の全アイテムの選択状態を反転"),
						4, SET_IN_FILE },
	{ "menu_last_page", E_J("jump to the last page in a menu",
				"メニューの最後のページに移動"), 4, SET_IN_FILE },
	{ "menu_next_page", E_J("goto the next menu page",
				"メニューの次のページに移動"), 4, SET_IN_FILE },
	{ "menu_previous_page", E_J("goto the previous menu page",
				    "メニューの前のページに移動"), 4, SET_IN_FILE },
	{ "menu_search", E_J("search for a menu item",
			     "メニューアイテムの検索"), 4, SET_IN_FILE },
	{ "menu_select_all", E_J("select all items in a menu",
				 "メニューの全アイテムを選択"), 4, SET_IN_FILE },
	{ "menu_select_page", E_J("select all items on this page of a menu",
				  "メニューの表示中の全アイテムを選択"),
						4, SET_IN_FILE },
	{ "monsters", E_J("the symbols to use for monsters",
			  "怪物の表示に使うシンボルのセット"),
						MAXMCLASSES, SET_IN_FILE },
	{ "msghistory", E_J("number of top line messages to save",
			    "メッセージラインを保存する行数"),
						5, DISP_IN_GAME },
//# ifdef TTY_GRAPHICS
# if defined(TTY_GRAPHICS) || defined(WIN32_GRAPHICS2)
	{"msg_window", E_J("the type of message window required",
			   "必要なメッセージウィンドウの種類"),1, SET_IN_GAME},
# else
	{"msg_window", E_J("the type of message window required",
			   "必要なメッセージウィンドウの種類"), 1, SET_IN_FILE},
# endif
	{ "name",     E_J("your character's name (e.g., name:Merlin-W)",
			  "あなたのキャラクターの名前 (例 name:Merlin-W)"),
						PL_NSIZ, DISP_IN_GAME },
	{ "number_pad", E_J("use the number pad","移動にテンキーを使う"), 1, SET_IN_GAME},
	{ "objects",  E_J("the symbols to use for objects",
			  "品物の表示に使うシンボルのセット"),
						MAXOCLASSES, SET_IN_FILE },
	{ "packorder", E_J("the inventory order of the items in your pack",
			   "所持品の種類ごとの表示順"),
...
# endif
#endif
	{ "pettype",  E_J("your preferred initial pet type",
			  "あなたの好きなペットの種類"), 4, DISP_IN_GAME },
	{ "pickup_burden", E_J("maximum burden picked up before prompt",
			       "物を拾う前に警告する重量レベル"),
						20, SET_IN_GAME },
	{ "pickup_types", E_J("types of objects to pick up automatically",
			      "自動的に拾う品物の種類"),
						MAXOCLASSES, SET_IN_GAME },
	{ "player_selection", E_J("choose character via dialog or prompts",
				  "対話的にキャラクターを選ぶ"),
						12, DISP_IN_GAME },
	{ "race",     E_J("your starting race (e.g., Human, Elf)",
			  "あなたの開始時の種族 (例 Human, Elf)"),
						PL_CSIZ, DISP_IN_GAME },
	{ "role",     E_J("your starting role (e.g., Barbarian, Valkyrie)",
			  "あなたの開始時の職業 (例 Barbarian, Valkyrie"),
						PL_CSIZ, DISP_IN_GAME },
	{ "runmode", E_J("display frequency when `running' or `travelling'",
			 "連続移動時の表示間隔の指定"),
						sizeof "teleport", SET_IN_GAME },
	{ "scores",   E_J("the parts of the score list you wish to see",
			  "スコアリストのうち見たい部分"),
						32, SET_IN_GAME },
	{ "scroll_amount", E_J("amount to scroll map when scroll_margin is reached",
			       "scroll_margin にかかった場合のマップのスクロール量"),
						20, DISP_IN_GAME }, /*WC*/
	{ "scroll_margin", E_J("scroll map when this far from the edge",
			       "端からこの位置まで来たらマップをスクロールする"), 20, DISP_IN_GAME }, /*WC*/
#ifdef MSDOS
	{ "soundcard", E_J("type of sound card to use","使用するサウンドカードの種類"), 20, SET_IN_FILE },
#endif
	{ "suppress_alert", E_J("suppress alerts about version-specific features",
				"バージョン依存の機能を使用する前に警告しない"),
						8, SET_IN_GAME },
	{ "tile_width", E_J("width of tiles","タイルの横幅"), 20, DISP_IN_GAME},	/*WC*/
	{ "tile_height", E_J("height of tiles","タイルの縦幅"), 20, DISP_IN_GAME},	/*WC*/
	{ "tile_file", E_J("name of tile file","タイルのファイル名"), 70, DISP_IN_GAME},	/*WC*/
	{ "traps",    E_J("the symbols to use in drawing traps",
			  "罠の表示に使うシンボルのセット"),
						MAXTCHARS+1, SET_IN_FILE },
	{ "vary_msgcount", E_J("show more old messages at a time",
			       "一度に表示するメッセージライン"), 20, DISP_IN_GAME }, /*WC*/
#ifdef MSDOS
	{ "video",    E_J("method of video updating","画面更新の方法"), 20, SET_IN_FILE },
#endif
#ifdef VIDEOSHADES
...
#endif
#ifdef WIN32CON
	{"subkeyvalue", E_J("override keystroke value",
			    "キーストロークのオーバーライド"), 7, SET_IN_FILE},
#endif
	{ "windowcolors", E_J("the foreground/background colors of windows",
			      "ウィンドウの文字色/背景色"),	/*WC*/
						80, DISP_IN_GAME },
	{ "windowtype", E_J("windowing system to use",
			    "使用するウィンドウシステム"), WINTYPELEN, DISP_IN_GAME },
	{ (char *)0, (char *)0, 0, 0 }
};
...
#else
	pline(E_J("%s can be set only from NETHACKOPTIONS or %s.",
		  "%sはNETHACKOPTIONSまたは%sの中でみ設定できます。"), optname,
			configfile);
#endif
}
...
const char *opts;
{
	if (!initial) {
		...
		pline(E_J("Bad syntax: %s.  Enter \"?g\" for help.",
			  "文法の誤り: %s (「?g」でヘルプ)"), opts);
	    return;
	}
#ifdef MAC
	...
	if(from_file)
	    raw_printf(E_J("Bad syntax in OPTIONS in %s: %s.",
			   "%s内のOPTIONSに文法の誤り: %s"), configfile, opts);
	else
	    raw_printf(E_J("Bad syntax in NETHACKOPTIONS: %s.",
			   "NETHACKOPTIONSに文法の誤り: %s"), opts);
...
{
	pline_The(E_J("%s option may not %sbe negated.",
		      "%sオプションは%sオフにできません"),
		optname,
		with_parameter ? E_J("both have a value and ",
				     "値の設定および") : "");
}
...
const char *optn;
{
	...
	if (rejectver) {
		if (!initial)
			You_cant(E_J("disable new feature alerts for future versions.",
				     "未来のバージョンに対する警告をオフにはできません。"));
		else {
			Sprintf(buf,
				E_J("\n%s=%s Invalid reference to a future version ignored",
				    "\n%s=%s 未来のバージョンに対する無効な参照"),
				optn, op);
			badoption(buf);
		}
		return 0;
	...
	if (!initial) {
		...
		pline(E_J("Feature change alerts disabled for NetHack %s features and prior.",
			  "NetHack %s以前の機能の変更に対する警告をオフにしました。"),
			buf);
	}
	return 1;
}
...
boolean tinitial, tfrom_file;
{
	...
	if (strlen(opts) > BUFSZ/2) {
		badoption(E_J("option too long","オプションが長すぎます"));
		return;
	}
	...
	if (match_optname(opts, "male", 4, FALSE)) {
		if(!initial && flags.female != negated)
			pline(E_J("That is not anatomically possible.",
				  "それは生物学的に不可能です。"));
		else
			flags.initgend = flags.female = negated;
		return;
	...
	if (match_optname(opts, fullname, 3, TRUE)) {
		if ((op = string_for_env_opt(fullname, opts, negated)) != 0) {
			...
			case 'N':
			    ...
			    break;
			default:
			    pline(E_J("Unrecognized pet type '%s'.",
				      "無効なペットの種類 '%s'"), op);
			    break;
		    }
...
# endif
	...
	if (match_optname(opts, "fruit", 2, TRUE)) {
		...
		if (!initial) {
		    ...
		    if (num >= 100) {
			pline(E_J("Doing that so many times isn't very fruitful.",
				  "そんなにたくさんの指定は実りあることとはいえない。"));
			return;
		    }
		}
goodfruit:
	...
	/* OBJ_NAME(objects[SLIME_MOLD]) won't work after initialization */
		if (!*pl_fruit)
		    nmcpy(pl_fruit, E_J("slime mold","スライムモルド"), PL_FSIZ);
		if (!initial)
		    (void)fruitadd(pl_fruit);
...
/*		if (!(opts = string_for_env_opt(fullname, opts, FALSE))) */
		...
		if (clash) {
			...
			pline(
		  E_J("Badoption - boulder symbol '%c' conflicts with a %s symbol.",
		      "無効なオプション - 大岩のシンボル '%c' は%sのシンボルと衝突しています"),
				opts[0], (clash == 1) ? E_J("monster","怪物") : E_J("warning","警告表示"));
		} else {
			/*
	...
	if (match_optname(opts, "pickup_types", 8, TRUE)) {
		...
		if (!op) {
			...
			use_menu = FALSE;
			Sprintf(qbuf, E_J("New pickup_types: [%s am] (%s)",
					  "新たな自動拾いの種別: [%s am] (%s)"),
				ocl, *tbuf ? tbuf : E_J("all","全て"));
			getlin(qbuf, abuf);
			op = mungspaces(abuf);
		    ...
		    if (use_menu) {
			(void) choose_classes_menu(E_J("Auto-Pickup what?",
					"どれを自動的に拾いますか？"), 1,
						   TRUE, ocl, tbuf);
			op = tbuf;
		    }
...
# endif /* NO_TERMS */
	...
	for (i = 0; boolopt[i].name; i++) {
		if (match_optname(opts, boolopt[i].name, 3, FALSE)) {
			...
			if (!boolopt[i].addr) {
			    if (!initial && !negated)
				pline_The(E_J("\"%s\" option is not available.",
					      "\"%s\"オプションは使用できません。"),
					boolopt[i].name);
			    return;
			}
...
# ifdef ASCIIGRAPH
				 ...
				 || (boolopt[i].addr) == &iflags.IBMrogue
# endif
# ifdef MAC_GRAPHICS_ENV
    ...
    char from_ch, to_ch;
{
	...
	pline(E_J("out of menu map space.","メニューキーの割り当てが多すぎます。"));
    else {
...
doset()
{
 ...
 add_menu(tmpwin, NO_GLYPH, &any, 0, 0, iflags.menu_headings,
		E_J("Booleans (selecting will toggle value):",
		    "有効/無効 (選択で反転):"), MENU_UNSELECTED);
	any.a_int = 0;
 ...
 add_menu(tmpwin, NO_GLYPH, &any, 0, 0, iflags.menu_headings,
		E_J("Compounds (selecting will prompt for new value):",
		    "引数を持つもの (選択で新しく設定)"),
		 MENU_UNSELECTED);
	...
	add_menu(tmpwin, NO_GLYPH, &any, 0, 0, ATR_NONE, "", MENU_UNSELECTED);
	add_menu(tmpwin, NO_GLYPH, &any, 0, 0, iflags.menu_headings,
		 E_J("Variable playground locations:",
		     "ファイルの保存パス:"), MENU_UNSELECTED);
	for (i = 0; i < PREFIX_COUNT; i++)
		doset_add_menu(tmpwin, fqn_prefix_names[i], 0);
#endif
	end_menu(tmpwin, E_J("Set what options?","どのオプションを設定しますか？"));
	need_redraw = FALSE;
	if ((pick_cnt = select_menu(tmpwin, PICK_ANY, &pick_list)) > 0) {
		...
		if (opt_indx < boolcount) {
		    ...
		    if (!special_handling(compopt[opt_indx].name,
							setinitial, fromfile)) {
			Sprintf(buf, E_J("Set %s to what?",
					 "%sに何を設定しますか？"), compopt[opt_indx].name);
			getlin(buf, buf2);
			if (buf2[0] == '\033')
			    continue;
...
boolean setinitial,setfromfile;
{
	...
	end_menu(tmpwin, E_J("Select menustyle:","メニューのスタイルの選択:"));
	...
	end_menu(tmpwin, E_J("Select encumbrance level:","重量レベルの選択:"));
	...
	end_menu(tmpwin, E_J("Select run/travel display mode:",
			     "連続移動時の表示モードの選択:"));
...
//#ifdef TTY_GRAPHICS
	...
	end_menu(tmpwin, E_J("Select message history display type:",
			     "メッセージの履歴の表示タイプを選択:"));
	...
	Sprintf(abuf, E_J("Select %s window placement relative to the map:",
			  "マップに対する%sウィンドウの配置を選択:"),
		msg ? E_J("message","メッセージ") : E_J("status","ステータス"));
	...
	end_menu(tmpwin, E_J("Select number_pad mode:",
			     "number_pad モードの選択:"));
	if (select_menu(tmpwin, PICK_ONE, &mode_pick) > 0) {
...
#else
	static const char *mhchoices[3] = {"強調", "反転", "下線"};
#endif /*JP*/
	...
	end_menu(tmpwin, E_J("How to highlight menu headings:",
			     "メニューの見出しの表示方法:"));
	if (select_menu(tmpwin, PICK_ONE, &mode_pick) > 0) {
		int mode = mode_pick->item.a_int - 1;
		switch(mode) {
	...
	static const char *action_titles[] = {
		"a", E_J("add new autopickup exception","自動拾い例外を追加"),
		"l", E_J("list autopickup exceptions","自動拾い例外の一覧"),
		"r", E_J("remove existing autopickup exception","自動拾い例外を削除"),
		"e", E_J("exit this menu","このメニューを終了"),
	};
ape_again:
	opt_idx = 0;
	...
	end_menu(tmpwin, E_J("Do what?","何を行いますか？"));
	...
	if (opt_idx == 0) {	/* add new */
		getlin(E_J("What new autopickup exception pattern?",
			   "自動拾い例外に何を追加しますか？"), &apebuf[1]);
		...
		for (pass = AP_LEAVE; pass <= AP_GRAB; ++pass) {
		    ...
		    any.a_void = 0;
		    add_menu(tmpwin, NO_GLYPH, &any, 0, 0, iflags.menu_headings,
				(pass == 0) ? E_J("Never pickup","常に拾わない") :
					      E_J("Always pickup","常に拾う"),
				MENU_UNSELECTED);
...
#else
		Sprintf(apebuf, (opt_idx == 1) ? "自動拾い例外の一覧" :
						 "どの自動拾い例外を削除しますか？");
#endif /*JP*/
...
dotogglepickup()
{
	...
	if (flags.pickup) {
	    oc_to_str(flags.pickup_types, ocl);
	    Sprintf(buf, E_J("ON, for %s objects%s","%sの品物%sに対してON"),
		    ocl[0] ? ocl : E_J("all","全て"),
#ifdef AUTOPICKUP_EXCEPTIONS
	...
	pline(E_J("Autopickup: %s.","自動拾い: %s"), buf);
	return 0;
}
...
const char *mapping;
{
	...
	if (sscanf(mapping, "\"%255[^\"]\"", text) == 1) {
		...
		*apehead = ape;
	    ...
	    raw_print(E_J("syntax error in AUTOPICKUP_EXCEPTION",
			  "AUTOPICKUP_EXCEPTION に文法の誤り"));
	    return 0;
	}
	return 1;
}
...
static const char *opt_intro[] = {
	"",
    E_J("                 NetHack Options Help:",
	"                 NetHack オプションのヘルプ:"),
	"",
#define CONFIG_SLOT 3	/* fill in next value at run-time */
	(char *)0,
#if !defined(MICRO) && !defined(MAC)
    E_J("or use `NETHACKOPTIONS=\"<options>\"' in your environment",
	"または `NETHACKOPTIONS=\"<options>\"' を環境変数に指定"),
#endif
    E_J("(<options> is a list of options separated by commas)",
	"(<options> はカンマで区切られたオプションのリスト)"),
#ifdef VMS
    E_J("-- for example, $ DEFINE NETHACKOPTIONS \"noautopickup,fruit:kumquat\"",
	"-- 例, $ DEFINE NETHACKOPTIONS \"noautopickup,fruit:kumquat\""),
#endif
    E_J("or press \"O\" while playing and use the menu.",
	"またはゲーム中に \"O\" を押してメニューで設定できます。"),
	"",
 E_J("Boolean options (which can be negated by prefixing them with '!' or \"no\"):",
     "有効/無効オプション (先頭に '!' または \"no\" を付けることで無効にできます):" ),
	(char *)0
};

static const char *opt_epilog[] = {
	"",
 E_J("Some of the options can be set only before the game is started; those",
     "いくつかのオプションはゲーム開始前にしか設定できず、" ),
 E_J("items will not be selectable in the 'O' command's menu.",
     "これらの項目は 'O' コマンドのメニューからは選択できません。" ),
	(char *)0
};
...
option_help()
{
    ...
    Sprintf(buf, E_J("Set options as OPTIONS=<options> in %s",
		     "%s に、OPTIONS=<options> の形式でオプションを設定します。"), configfile);
    ...
    /* Compound options */
    putstr(datawin, 0, E_J("Compound options:","値を持つオプション:"));
    for (i = 0; compopt[i].name; i++) {
...
char *str;
{
	...
	if (user_specified) {
...
#ifdef JP
			if (!strcmp(JOBJ_NAME(objects[i]), pl_fruit)) {
				found = TRUE;
				break;
			}
#endif /*JP*/
		...
		if (found || numeric ||
#ifdef JP
		    !strncmp(str, "呪われた", 8) ||
		    !strncmp(str, "呪われていない", 14) ||
		    !strncmp(str, "祝福された", 10) ||
		    !strncmp(str, "食べかけの", 10) ||
		    !strcmpr(str, "の缶詰") ||
		    !strcmpr(str, "の死体") ||
		    !strcmpr(str, "の卵") ||
#endif /*JP*/
			...
			{
				Strcpy(buf, pl_fruit);
				Strcpy(pl_fruit, E_J("candied ","飴細工の"));
