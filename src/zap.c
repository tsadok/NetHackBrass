...
const char * const flash_simple_name[] = {
#ifndef JP
	/* Wands 0-9 */
	"missile", "bolt", "bolt", "ray", "ray", "lightning", "", "", "", "",
	/* Spells 10-19 */
	"missile", "fireball", "blizzard", "ray", "ray", "lightning", "", "", "", "",
	/* Breathes 20-29 */
	...
	"矢", "炎", "冷気", "光線", "光線", "稲妻", "", "", "", "",
	/* Spells 10-19 */
	"矢", "ファイアボール", "凍気", "光線", "死の指", "稲妻", "", "", "", "",
	/* Breathes 20-29 */
	"ブレス", "ブレス", "ブレス", "ブレス", "ブレス", "ブレス", "ブレス", "ブレス", "ブレス", ""
#endif /*JP*/
};
...
struct obj *otmp;
{
	...
	case SPE_EXTRA_HEALING:
		...
		mtmp->mhp += d( 6+(Role_if(PM_HEALER) ? 1 : 0)+
...
register struct obj *obj;
{
	...
	switch(obj->otyp) {
		...
		case WAN_MAINTENANCE: {
			...
			else if (objects[otmp->otyp].oc_descr_idx == CLOAK_OF_PROTECTION)
...
int x1, y1, x2, y2;
{
	...
	dx = x2 - x1;
	dy = y2 - y1;
	bp->vx = (dx == 0) ? 0 : ((dx < 0) ? -1 : 1);
	bp->vy = (dy == 0) ? 0 : ((dy < 0) ? -1 : 1);
	if (dx < 0) dx = -dx;
	if (dy < 0) dy = -dy;
...
/*test*/return bhitcore2(sx,sy, ddx, ddy, range, weapon, fhitm, fhito, obj, TRUE);
}
...
boolean youshoot;			/* TRUE:you shot it */
{
	...
	if ((weapon & BHIT_GLYPHMASK) == BHIT_GLYPH_FLASH) {
	    tmp_at(DISP_BEAM, cmap_to_glyph(S_flashbeam));
	} else if ((weapon & BHIT_GLYPHMASK) == BHIT_GLYPH_OBJ)
	...
	while(range-- > 0) {
	    ...
	    /* drawbridge handling */
	    if ((weapon & BHIT_ZAPPEDWAND) && find_drawbridge(&x,&y))
		switch (obj->otyp) {
		    case WAN_OPENING:
		    case SPE_KNOCK:
	    ...
	    if (!youshoot && bhitpos.x==u.ux && bhitpos.y==u.uy)
		 mtmp = &youmonst;
	    else mtmp = m_at(bhitpos.x, bhitpos.y);
	    if (mtmp) {
#ifdef MONSTEED
		...
		if (!((weapon & BHIT_PASSINVIS) && mtmp->minvis)) {
			...
			if ((weapon & BHIT_SENSEINVIS) &&
			    cansee(bhitpos.x,bhitpos.y) && !canspotmons(mtmp))
				map_invisible(bhitpos.x, bhitpos.y);
		...
		case SPE_FORCE_BOLT:
			...
			if (levl[bhitpos.x][bhitpos.y].doormask == D_BROKEN
			    ...
			    shopdoor = TRUE;
			    add_damage(bhitpos.x, bhitpos.y, youshoot ? 400L : 0L);
			}
		    }
		    break;
		default:
...
struct monst *
bhitcore2(startx,starty, ddx,ddy, range, weapon, fhitm,fhito, obj, youshoot)
int startx, starty;			/* starting position */
int ddx,ddy,range;			/* direction and range */
int weapon;				/* see values in hack.h */
int FDECL((*fhitm), (MONST_P, OBJ_P)),	/* fns called when mon/obj hit (return value: ) */
...
int x1,y1,x2,y2,x3,y3,beam_type;
{
	...
	if (d2 >= 4) d2--;
	return (beam_type << 4) + (z2g[(d1 << 3) + d2] - S_vbeam) + GLYPH_ZAP_OFF;
}
...
/*    coord save_bhitpos;*/
    ...
    zapglyph = GLYPH_ZAP_OFF; /* temp */
    tmp_at(DISP_BEAM, zapglyph);
//    tmp_at(DISP_BEAM, zapdir_to_glyph(dx, dy, abstype));
	...
	if(isok(sx,sy) && (lev = &levl[sx][sy])->typ) {
		...
		if(ZAP_POS(lev->typ) || cansee(lsx,lsy)) {
		    zgtmp = zap_to_glyph2(lsx, lsy, sx, sy, nx, ny, abstype);
		    if (zgtmp != zapglyph) {
			zapglyph = zgtmp;
			tmp_at(DISP_CHANGE, zapglyph);
		    }
		    tmp_at(sx,sy);
		}
	...
	if (type != ZT_SPELL(ZT_FIRE))
	    range += zap_over_floor(sx, sy, type, &shopdamage) * bhitpos.threshold;
	...
	if (mon) {
		...
		blocked = elem_hits_shield(mon, abstype+1, fltxt);
		...
		if (mon_reflects(mon, (char *)0)) {
			...
			if (canseemon(mon)) {
			    ...
			    pline(E_J("%s absorbs the deadly %s!",
				      "%sは致死の%sを吸収している！"), Monnam(mon),
				  type == ZT_BREATH(ZT_DEATH) ?
					E_J("blast","ブレス") : E_J("ray","光線"));
...
#define oresist_disintegration(obj) \
		...
		range -= 2 * bhitpos.threshold;
		blocked = elem_hits_shield(&youmonst, abstype+1, fltxt);
		if (blocked) goto skip_hit;
#ifndef JP
		...
		if (Reflecting) {
...
#else
		    	(void) ureflects("だが、%sはあなたの%sで反射した！",
				flash_simple_name[(type <= -30) ? abstype : abs(type)]);
#endif /*JP*/
		    } else
		    ...
		    bhitpos.vy = -bhitpos.vy;
		    shieldeff(sx, sy);
		    switch (abstype) {		/* reflector gets damaged */
			case ZT_MAGIC_MISSILE:
			case ZT_SLEEP:
			case ZT_POISON_GAS:
			case ZT_PARALYSIS:
...
skip_hit:
 ...
 make_bounce:
		...
		switch(bounce) {
		case 0: dx = -dx; bhitpos.vx = -bhitpos.vx; /* fall into... */
		case 1: dy = -dy; bhitpos.vy = -bhitpos.vy; break;
		case 2: dx = -dx; bhitpos.vx = -bhitpos.vx; break;
		}

		if(isok(sx,sy) && cansee(sx,sy)) {
		    bb = bhitpos;
		    bresenham_step(&bb);
		    zgtmp = zap_to_glyph2(lsx, lsy, sx, sy, bb.x, bb.y, abstype);
		    if (zgtmp != zapglyph) {
			zapglyph = zgtmp;
			tmp_at(DISP_CHANGE, zapglyph);
		    }
...
//		tmp_at(DISP_CHANGE, zapdir_to_glyph(dx,dy,abstype));
    ...
    if (shopdamage)
	pay_for_damage(abstype == ZT_FIRE ?  E_J("burn away",	"扉を燃やした") :
		       abstype == ZT_COLD ?  E_J("shatter",	"扉を砕いた") :
		       abstype == ZT_DEATH ? E_J("disintegrate","扉を分解した") :
...
boolean *shopdamage;
{
	...
	if(closed_door(x, y)) {
		...
		def_case:
			...
			pline("扉は%s%sを吸収した！",
			      (type < 0) ? "" : "あなたの",
			      abs(type) < ZT_SPELL(0) ? flash_simple_name[abs(type)] :
