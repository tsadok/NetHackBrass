...
STATIC_OVL void FDECL(gaishu2xy, (int, int *, int *));
...
wallify_map()
{
	...
	for(y = 0; y < ROWNO; y++)
		...
		for(yy = y - 1; yy <= y+1; yy++)
			...
			if(isok(xx,yy) && !IS_STWALL(levl[xx][yy].typ)/*levl[xx][yy].typ == ROOM*/) {
	...
	lev_init	*init_lev;
{
	...
	mkmap_sub(bg_typ, fg_typ, N_P1_ITER, N_P2_ITER,
		  (smooth) ? N_P3_ITER : 0);
...
void
mkmap_sub(bg_typ, fg_typ, pass1, pass2, pass3)
schar bg_typ, fg_typ;
...
int *x, *y;
{
	if (g < WIDTH) {
		*x = g;
		*y = 0;
	} else if (g < WIDTH+HEIGHT) {
		*x = WIDTH-1;
		*y = g-WIDTH;
	} else if (g < WIDTH*2+HEIGHT) {
		*x = WIDTH*2+HEIGHT-g-1;
		*y = HEIGHT-1;
	} else if (g < WIDTH*2+HEIGHT*2) {
		*x = 0;
		*y = WIDTH*2+HEIGHT*2-g-1;
	} else {				// error
		*x = 0;
		*y = 0;
	}
...
/* vector to 8-direction */
int
dxdy2dir(dx, dy)
int dx, dy;
{
	if (dx==0 && dy==0) return 6; /* vertical? return downward instead */
	if (dx<0 && dy<=0) {
	    dx = -dx;
	    dy = -dy;
	    if (dx > dy) return (dx >  dy*2) ? 0 : 1;
	    else	 return (dy <= dx*2) ? 1 : 2;
	}
	if (dx>=0 && dy<0) {
	    dy = -dy;
	    if (dy > dx) return (dy >  dx*2) ? 2 : 3;
	    else	 return (dx <= dy*2) ? 3 : 4;
	}
	if (dx>0 && dy>=0) {
	    if (dx > dy) return (dx >  dy*2) ? 4 : 5;
	    else	 return (dy <= dx*2) ? 5 : 6;
	}
	/*if (dx<=0 && dy>0) */{
	    dx = -dx;
	    if (dy > dx) return (dy >  dx*2) ? 6 : 7;
	    else	 return (dx <= dy*2) ? 7 : 0;
	}
}

void mkriver(bg_typ, fg_typ)
schar	bg_typ, fg_typ;
{
	int sx, sy;
	int dx, dy;
	...
	i = rn2(WIDTH*2+HEIGHT*2);
//	j = (i + rn2(WIDTH+HEIGHT) + (WIDTH+HEIGHT)/2) % (WIDTH*2+HEIGHT*2);
	j = (i + WIDTH+HEIGHT) % (WIDTH*2+HEIGHT*2);
	gaishu2xy(i, &sx, &sy);
	gaishu2xy(j, &dx, &dy);
	dir = dxdy2dir(dx-sx, dy-sy);

	while (1) {
		for (i=-2; i<=2; i++) {
		  for (j=-3; j<=3; j++) {
		    if (!isok(sx+j,sy+i)) continue;
		    if ((bg_typ == INVALID_TYPE || levl[sx+j][sy+i].typ == bg_typ) &&
			rn2(j*j+i*i) < rn2(4)+1)
			levl[sx+j][sy+i].typ = fg_typ;
		  }
		}
		switch (rn2(6)) {
			case 0:	dir++; break;
		...
		dir = (dir+8) & 7;
		if (!isok(sx+xdir[dir], sy+ydir[dir])) {
			if (abs(dx-sx)+abs(dy-sy) < 10) break;
			dir = dxdy2dir(dx-sx, dy-sy);
		}
		sx += xdir[dir];
		sy += ydir[dir];
...
/*
	make a random shaped space
 */
void mkrndspace(ix, iy, area, diagonal, fg, bg)
int ix;		/* starting point */
int iy; 	/* starting point */
int area;
int diagonal;
schar fg;	/* fill with this */
schar bg;	/* background to be updated (INVALID_TYPE: all) */
{
#define MAXSTK (COLNO*ROWNO)
#define HATEEDGE 3
	char bx[MAXSTK];
	char by[MAXSTK];
	short bm[MAXSTK];
	...
	while (maxoo) {
	    /* select random position weighted by "mass" */
	    index = indexstart;
	    for (j = rnd(maxoo); j>0; j -= bm[index++]);
	    index--;
	    /* find unfilled cell */
	    prob = 0; cnt = 0;
	    for (j=0; j<8; j++) {
	        x = bx[index] + xdir[j];
	        y = by[index] + ydir[j];
		if (isok(x, y) && levl[x][y].typ != fg) {
			if (bg != INVALID_TYPE && bg != levl[x][y].typ) continue;
			i = 100;
			kx[cnt] = x;
			ky[cnt] = y;
			/* diagonal */
			if (!diagonal && xdir[j] && ydir[j]) continue;
			/* considering aspect ratio */
			if (xdir[j] && !ydir[j]) i *= 5;
			/* keep off from edge */
			if (x <= HATEEDGE) i >>= x;
			if (x > (COLNO-HATEEDGE)) i >>= (COLNO-x);
			if (y < HATEEDGE) i >>= (y+1);
			if (y > (ROWNO-HATEEDGE)) i >>= (ROWNO-y);
			if (!i) i++;
			kp[cnt] = i;
			prob += i;
			cnt++;
	    ...
	    /* move certain amount of "mass" to adjacent cell */
	    if (prob) {
		if (bsize >= MAXSTK) return;
		for (i=0, j=rnd(prob); j>0; j-=kp[i++]);
		i--;
		m = bm[index]-rn2((bm[index]+1)/4);
//		m = bm[index]-rn2((bm[index]+1)/2);
//		m = bm[index]-rn2(rnd(bm[index]));
		if (m<=0) {
			impossible("mkrndspace(): Illegal mass movement");
			return;
		}
		bm[index] -= m;
		x = kx[i];
		...
		if (index+1 >= bsize) {
			...
			bm[index+1] += bm[index];
			bm[index] = 0;
			if (index == indexstart) indexstart++;
		}
	    }
	}
#undef MAXOOKISA
