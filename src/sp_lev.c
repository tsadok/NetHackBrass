...
STATIC_DCL void NDECL(cleanup_mons);
...
register int humidity;
{
	...
	if (humidity & DRY) {
		...
		typ == GROUND || typ == GRASS || typ == CARPET)
		return TRUE;
	}
	if (humidity & WET) {
...
struct mkroom	*croom;
{
	...
	else mtmp = makemon(pm, x, y, MM_MONSTEEDOK/*NO_MM_FLAGS*/);
	...
	if (mtmp) {
		...
		"create_monster: mon has an appearance, \"%s\", but no type",
			...
			for (i = 0; i < MAXPCHARS; i++)
			    if (!strcmp(E_J(defsyms[i].explanation,
					    defsyms[i].eexplanation),
					m->appear_as.str))
				break;
			if (i == MAXPCHARS) {
			    impossible(
...
cleanup_mons()
{
	...
	for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
	    /* clear the temporal serial number */
	    mtmp->meating = 0;
	}
}
...
struct mkroom	*croom;
{
	...
	/* Sokoban prize special handling */
...
#else
	{   /* 英語名のアーティファクトを日本語名に変換 */
	    const char *ntmp;
	    short otyp;
	    ntmp = artifact_name(o->name.str, &otyp);
	    otmp = oname(otmp, ntmp ? ntmp : o->name.str);
	}
#endif /*JP*/
	...
	switch(o->containment) {
	    ...
	    /* nothing */
	    case 0: break;
	    /* monster picks it up... handled later */
	    case 3: break;
	    /* buried */
	    case 4: break;

	    default:
		if (o->containment < 0) break;
	...
	/* monster picks up the object */
	if (o->containment == 3 || o->containment < 0) {
	    struct monst *mtmp;
	    if (o->containment < 0) {
		for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
		    if (mtmp->meating == -(o->containment)) break;
		}
	    } else
		mtmp = m_at(x, y);
	    if (mtmp) {
		...
		mpickobj(mtmp, otmp);
		m_dowear(mtmp, TRUE);	/* mon would use the given equipment */
	    } else
		impossible("no monster to pick up obj");
		stackobj(otmp);
	} else if (o->containment == 4) {
	    bury_an_obj(otmp);
	} else
...
dlb *fd;
{
	...
	for (i=0;i<nrooms;i++) {
		...
		if ((n = r->nmonster) != 0) {
		    ...
		    j = 0;
		    while(n--) {
			r->monsters[j] = New(monster);
			load_one_monster(fd, r->monsters[j++]);
		    }
		} else
		    r->monsters = 0;
...
dlb *fd;
{
    ...
    if (!init_lev.init_present && filling != -2) { /* don't init if mkmap() has been called */
	...
	Fread((genericptr_t) &ysize, 1, sizeof(ysize), fd);
					/* size in Y */
	if (halign == EDGE) halign = rn2(2) ? LEFT : RIGHT;
	if (valign == EDGE) valign = rn2(2) ? TOP  : BOTTOM;
	switch((int) halign) {
	    ...
	    default:	    xstart = 2+rn2(x_maze_max-2-xsize);		break;
	}
	ymin = (rndvault) ? 0 : 2;
	switch((int) valign) {
	    case TOP:	    ystart = ymin+1/*3*/;			break;
	    case CENTER:    ystart = ymin+((y_maze_max-ymin-ysize)/2);	break;
	    case BOTTOM:    ystart = y_maze_max-ysize-1;		break;
	    default:	    ystart = ymin+rn2(y_maze_max-ymin-ysize);	break;
	}
	if (!(xstart % 2)) xstart++;
	if (!(ystart % 2)) ystart++;
	...
	while(n--) {
		...
		if (rndvault && !proom && tmpregion.rirreg == 2 &&
			...
			for(x = xstart; x < xstart+xsize; x++) {
			    if (Map[x][y]) {
				levl[x][y].roomno = nroom+ROOMOFFSET;
				levl[x][y].lit = tmpregion.rlit;
			    }
		    ...
		    add_room(xstart+1, ystart+1, xstart+xsize-2, ystart+ysize-2,
			     FALSE, tmpregion.rtype, TRUE);
		    proom->rlit = tmpregion.rlit;
		    proom->irregular = TRUE;
		    proom->rndvault = 1;
		    continue;
		}
		...
		/* dummy region to avoid putting doors in
		   specified area. Do not make an actual room */
		if(tmpregion.rtype == REGNODOOR) {
		    tmpregion.rtype = OROOM;
		    set_wall_property(tmpregion.x1, tmpregion.y1,
				      tmpregion.x2, tmpregion.y2, W_NODOOR);
		    continue;
		}
		...
		if(tmpregion.rirreg) {
			...
			add_subroom(proom, min_rx, min_ry, max_rx, max_ry,
				    FALSE, tmpregion.rtype, TRUE);
	...
	while(n--) {
		...
		if (rndvault) {
		    if (proom && levl[x][y].edge &&
			(levl[x][y].roomno == (proom - rooms + ROOMOFFSET)) &&
			(proom->lx == x+1 || proom->hx == x-1 ||
			 proom->ly == y+1 || proom->hy == y-1)) {
			add_door(x, y, proom);
			continue;
		    } else {
			/* The doors inside a random vault should be
			   added to subrooms only. */
			croom = &subrooms[0];
		    }
		}
	...
	while(n--) {
		...
		croom = (levl[x][y].roomno >= ROOMOFFSET) ?
			&rooms[levl[x][y].roomno - ROOMOFFSET] :
			(struct mkroom *)0;
    ...
    if (nwalk_sav && (mapcount > (int) (mapcountmax / 10))) {
	    ...
	    for(x = rnd((int) (12 * mapfact) / 100); x; x--) {
		    maze1xy(&mm, WET|DRY);
		    (void) makemon((struct permonst *) 0, mm.x, mm.y, MM_MONSTEEDOK);
	    }
...
/* finddpos for random vault */
boolean
finddpos_special(cc, xl,yl,xh,yh)
coord *cc;
xchar xl,yl,xh,yh;
{
	int i = 0;
	...
	for(x = xl; x <= xh; x++) for(y = yl; y <= yh; y++)
	    if (okdoor(x, y) || IS_DOOR(levl[x][y].typ) ||
		(levl[x][y].typ == CORR)) {
		/* use Map to work area... */
		Map[i][0] = x;
		Map[i][1] = y;
		i++;
		if (i >= COLNO) break;
...
/* sort_rooms() would break the link between rooms[] and doors[],
   so it is necessary to refresh the link if you created any room doors
   before sort_rooms(). */
void
sort_doors()
{
	...
	for (i=0; i<nsubroom; i++) {
	    croom = &subrooms[i];
	    k = croom->fdoor;
	    croom->fdoor = newindex;
	    for (j=croom->doorct; j; j--) {
		doors[newindex].x = dcpx[k];
		doors[newindex].y = dcpy[k];
		newindex++;
		k++;
	    }
	}
