...
int lev;
{
	...
	*/
	const long exptbl[30] = {
		     75L,     150L,     300L,     600L,    1200L,
		   2500L,    5000L,   10000L,   20000L,   30000L,
		  40000L,   50000L,   70000L,   90000L,  120000L,
		 150000L,  200000L,  250000L,  300000L,  400000L,
		 500000L,  700000L,  900000L, 1200000L, 1500000L,
		1800000L, 2100000L, 2400000L, 2700000L, 3100000L
	};
	if (lev <  1) lev =  1;
	if (lev > 30) lev = 30;
	return exptbl[lev-1];
...
const char *drainer;	/* cause of death, if drain should be fatal */
{
	...
	if (u.ulevel > 1) {
		pline(E_J("%s level %d.","%s、レベル%ld。"), Goodbye(), u.ulevel--);
...
//	u.uhp -= num;
	...
	/* 
	 * Minor change .. when level drained, you drop to half way point
	 * of previous level. -SW
	 */
	u.uexp = newuexp(u.ulevel)-(newuexp(u.ulevel)-newuexp(u.ulevel-1))/2;
...
{		/*	(false for potion of gain level)      */
	...
	if (!incr) You_feel(E_J("more experienced.","より経験を積んだ。"));
	if (u.ulevel < MAXULEV) {
	    if (incr) {
	    ...
	    ++u.ulevel;
	    pline(E_J("Welcome to experience level %d.",
		      "レベル%dへようこそ。"), u.ulevel);
	    if (u.ulevelmax < u.ulevel) {
		u.ulevelmax = u.ulevel;
		enhance_weapon_skill(TRUE);		/* enhance weapon skill */
	    }
