...
#else
static const char *levitate[3]	= { "•‚‚©‚Ñ",	"‚®‚ç‚Â‚¢‚½",	"‚®‚ç‚Â‚«" };
static const char *flys[3]	= { "”ò‚Ñ",	"‚Ó‚ç‚Â‚¢‚½",	"‚Ó‚ç‚Â‚«" };
static const char *flyl[3]	= { "”ò‚Ñ",	"‚æ‚ë‚ß‚¢‚½",	"‚æ‚ë‚ß‚«" };
static const char *slither[3]	= { "”‡‚¢",	"‚Ì‚½‚¤‚Á‚½",	"‚Ì‚½‚¤‚¿" };
static const char *ooze[3]	= { "—¬‚ê",	"k‚¦‚½",	"k‚¦"	   };
static const char *immobile[3]	= { "“]‚°",	"—h‚ê“®‚¢‚½",	"—h‚ê“®‚«" };
static const char *crawl[3]	= { "”‡‚¢",	"‚Ó‚ç‚Â‚¢‚½",	"‚Ó‚ç‚Â‚«" };
#endif /*JP*/
...
#else
	int capitalize = 2 - (!strcmp(eos((char *)def)-2, "‚½"));
#endif /*JP*/
...
struct attack *mattk;
{
    ...
    case PM_SALAMANDER:
	what = E_J("already on fire","‚Í‚à‚¤”R‚¦‚Ä‚¢‚é");
	break;
    case PM_WATER_ELEMENTAL:
    case PM_FOG_CLOUD:
    case PM_STEAM_VORTEX:
	what = E_J("boiling","‚Í•¦“«‚µ‚Ä‚¢‚é");
	break;
    case PM_ICE_VORTEX:
    case PM_GLASS_GOLEM:
	what = E_J("melting","‚Í—n‚¯‚Ä‚¢‚é");
	break;
    case PM_STONE_GOLEM:
    case PM_CLAY_GOLEM:
    case PM_GOLD_GOLEM:
	...
	what = E_J("heating up","‚Í”M‚¹‚ç‚ê‚½");
	break;
    default:
	what = (mattk->aatyp == AT_HUGS) ? E_J("being roasted", "‚Í‰Š‚É•ï‚Ü‚ê‚Ä‚¢‚é") :
					   E_J("on fire","‚É‰Î‚ª‚Â‚¢‚½");
	break;
    }
    return what;
...
/* pxlth in permonst structure is only used by 5 monsters,
   so move it here rather than placing 0 for almost all monsters */
int
pxlth(mndx)
int mndx;
{
	switch (mndx) {
	    ...
	    case PM_HIGH_PRIEST:
		return sizeof(struct epri);
	    case PM_SHOPKEEPER:
		return sizeof(struct eshk);
	    case PM_GUARD:
		return sizeof(struct egd);
	    default:
		break;
	}
	return 0;
